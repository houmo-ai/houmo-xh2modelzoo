...
if TYPE_CHECKING:
    from xhmodel_merak.xh_llm.models.spark_xh import XHSparkMoeHMONNXModel
...
def main(args):
    ...
    hmonnx_model: XHSparkMoeHMONNXModel = AutoLLMHONNXModel.from_pretrained(model_meta_file)
    assert type(hmonnx_model).__name__ == "XHSparkMoeHMONNXModel", (
        f"Expected model type XHSparkMoeHMONNXModel, but got {type(hmonnx_model).__name__}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="work_dirs/xh2a_ipt-30b_w8a8h1_sefp_256_2k/hmquant_xh2_ipt-30b_w8a8_256_2k/golden_meta_info.json",
    )
