...
from xhmodel_merak.xh_llm.models.spark_xh import gptqmodel_wrap
from xhmodel_merak.xh_llm.models.spark_xh.spark import IPTConfig, IPTForCausalLM


def get_wikitext2(nsamples, seqlen):
    traindata = load_dataset("wikitext", "wikitext-2-raw-v1", split="train").filter(lambda x: len(x["text"]) >= seqlen)
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="data/models/ipt_30b_2/", help="fp model path")
    parser.add_argument(
        "--out",
        type=str,
        default=None,
...
def main():
    ...
    if not args.skip_quantize:
        ...
        model.model.unfuse_experts(use_padding_expert=False)
