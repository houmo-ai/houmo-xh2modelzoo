...
def _compute_rotary_cache(inv_freq: torch.Tensor, attention_scaling: float, max_seq_len: int) -> tuple[torch.Tensor, torch.Tensor]:
...
def _convert_gemma4_rmsnorm(hf_norm: nn.Module) -> XHRMSNorm:
    ...
    with torch.no_grad():
        new_norm.weight.data.copy_(hf_norm.weight.data.detach().to(new_norm.weight.dtype) + 1.0)
...
class Gemma4AssistantSelfAttention(nn.Module):
    def __init__(self, hf_attn: nn.Module, layer_type: str):
        ...
        default_head_dim = self.config.global_head_dim if layer_type == "full_attention" else self.config.head_dim
        ...
        if layer_type == "full_attention":
            self.num_key_value_heads = int(
                getattr(self.config, "num_global_key_value_heads", self.config.num_key_value_heads)
            )
        else:
        ...
        self.scaling = 1.0
        self.attn_logit_softcapping = getattr(hf_attn.config, "attn_logit_softcapping", None)
    ...
    def __init__(
        ...
        for layer_type in ("sliding_attention", "full_attention"):
            ...
            if inv_freq is None or attention_scaling is None:
                continue
            cos, sin = _compute_rotary_cache(inv_freq, attention_scaling, max_position_embeddings)
...
class Gemma4AssistantDraftModule(nn.Module):
    def __init__(
        ...
        super().__init__()
        assistant_config_dict = load_json_file(Path(assistant_model_dir) / "config.json")
        ...
        self.use_ordered_embeddings = bool(assistant_config_dict.get("use_ordered_embeddings", False))
        if self.use_ordered_embeddings:
            raise ValueError("Ordered assistant embeddings are not supported in this export path yet.")
...
class XHGemma4AssistantDraftModel(BaseModel):
    ...
    def prepare_inputs(self, data=None):
        ...
        local_window = aligned(int(target_text_cfg.get("sliding_window", 1024)) + input_sequence_length - 1, 16)
        full_heads = int(target_text_cfg.get("num_global_key_value_heads", target_text_cfg["num_key_value_heads"]))
        full_head_dim = int(target_text_cfg.get("global_head_dim", target_text_cfg["head_dim"]))
        ...
        if data is None:
            data = dict(
                ...
                local_attention_mask=torch.zeros((1, 1, input_sequence_length, local_window), dtype=dtype),
                global_attention_mask=torch.zeros((1, 1, input_sequence_length, max_sequence_length), dtype=dtype),
                shared_key_cache_sliding=torch.zeros(
                    (1, int(target_text_cfg["num_key_value_heads"]), local_window, int(target_text_cfg["head_dim"])),
                    dtype=dtype,
                ),
