...
def run_stateful_align_case(output_dir: Path, args: argparse.Namespace) -> dict:
    align_dir = output_dir / "08_stateful_streaming_align"
    script = Path(__file__).with_name("voxcpm2_stateful_streaming_align.py")
    command = [
        sys.executable,
        str(script),
        "--work_dir",
    ...
    report_path = align_dir / "stateful_streaming_alignment_report.json"
...
def main(args: argparse.Namespace) -> None:
    ...
    # The alignment subprocess loads its own pipeline, so release this suite's
    # models first to keep GPU memory usage bounded.
    del pipeline
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    summary["stateful_align"] = run_stateful_align_case(output_dir, args)
