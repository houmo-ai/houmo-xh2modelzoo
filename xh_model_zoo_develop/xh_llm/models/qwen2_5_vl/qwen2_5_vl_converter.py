...
class Qwen2_5_VLConverterXH2a(BaseLLMConverter):
    target_device = DeviceType.XH2a

    def __init__(self, config: BaseLLMConverterConfig):
        super().__init__()
        self.config = config
        self.work_dir = None
        self.pad_token_id = 0
    ...
    def _convert(self, hf_model_path: str, output_dir: str):
        ...
        with torch.no_grad():
            image_embeds = visual.forward_ori(
        ...
        if not Path(prefill_onnx_file).exists():
            ...
            onnx_input_names = BaseLLMConverter.xh1_hmonnx_compatible(onnx_input_names)
