...
# Description:
#   Qwen2.5-VL model definition adapted for the xh2 model zoo (xh2modelzoo).
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        Args:
            ...
            window_index (`torch.Tensor` of shape `(seq_len,)`):
                The temporal, height and width of feature shape of each image in LLM.
            window_mask (`torch.Tensor` of shape `(seq_len, seq_len)`):
        Returns:
            `torch.Tensor`: hidden_states.
        """
        _, _, th, tw = hidden_states.shape
        grid_thw = torch.tensor([[1, th // 14, tw // 14]])
    ...
    def forward_ori(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        """
        Args:
            hidden_states (`torch.Tensor` of shape `(seq_len, hidden_size)`):
                The final hidden states of the model.
...
# 放到你的 Qwen2_5_VLForConditionalGeneration 定义内，作为 @classmethod 覆写
    ...
    def forward(
        ...
        window_indexes: Optional[torch.LongTensor] = None,
        window_masks: Optional[torch.LongTensor] = None,
        ...
        if inputs_embeds is None:
            ...
            if hm_pixel_values is not None:
                # hm_pixel_values = hm_pixel_values.type(self.visual.dtype)
                image_embeds = list()
                for i in range(len(hm_pixel_values)):
                    image_embeds.append(
                        self.visual(
                            hm_pixel_values[i].type(self.visual.dtype).to(self.visual.device),
                            window_indexes[i].to(self.visual.device),
                            window_masks[i].type(self.visual.dtype).to(self.visual.device),
                ...
                image_embeds = self.visual.forward_ori(pixel_values, grid_thw=image_grid_thw)
                n_image_tokens = (input_ids == self.config.image_token_id).sum().item()
                n_image_features = image_embeds.shape[0]
                if n_image_tokens != n_image_features:
                    raise ValueError(
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        window_indexes = list()
        window_masks = list()
        for image_grid_thw in model_inputs["image_grid_thw"]:
            ...
            attention_mask = torch.full(
                [1, seq_len, seq_len],
                torch.finfo(pixel_values.dtype).min,
                device=pixel_values.device,
            ...
            window_masks.append(attention_mask)

        # Qwen2-5-VL position_ids are prepareed with rope_deltas in forward
        model_inputs["position_ids"] = None
        model_inputs["hm_pixel_values"] = hm_pixel_values
        model_inputs["window_indexes"] = window_indexes
        model_inputs["window_masks"] = window_masks
