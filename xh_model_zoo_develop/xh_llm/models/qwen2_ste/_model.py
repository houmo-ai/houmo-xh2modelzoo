...
# File: _model.py
# Description:
#   Qwen2 STE quantization model implementation adapted for the xh2 model zoo (xh2modelzoo).
...
import sys
import importlib
sys.path.append("/data01/home/xuchen/.cache/huggingface/modules/transformers_modules/gte_qwen2_1.5b_inst/") # modeling_qwen.py
# from modeling_qwen import (  # Qwen2SdpaAttention,
#     Qwen2Attention,
#     Qwen2DecoderLayer,
#     Qwen2ForCausalLM,
#     Qwen2RMSNorm,
...
Qwen2RMSNorm = modeling_qwen.Qwen2RMSNorm
Qwen2RotaryEmbedding = modeling_qwen.Qwen2RotaryEmbedding
Qwen2SdpaAttention = modeling_qwen.Qwen2SdpaAttention
Qwen2Model = modeling_qwen.Qwen2Model
Qwen2ForCausalLM = modeling_qwen.Qwen2ForCausalLM
...
class RotaryMatrixCache(nn.Module):
    ...
    def __init__(self, rotary_emb: Qwen2RotaryEmbedding, max_seq_len=4096):
        super().__init__()
...
class _Qwen2RotaryEmbedding(DynamicModule):
...
class _Qwen2Attention(DynamicModule):
    ...
    def _setup(self, cfg: Optional[Dict] = None):
        ...
        # self.key_group_broadcast_matmul = xhnn.GroupBroadcastMatMul()
        # self.value_group_broadcast_matmul = xhnn.GroupBroadcastMatMul()
...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen2DecoderLayer: "Qwen2DecoderLayer"})
class _Qwen2DecoderLayer(DynamicModule):
    def forward(
        self,
        hidden_states: torch.Tensor,
        past_seq_length: Optional[Tensor] = None,
...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen2Model: "Qwen2Model"})
class _Qwen2Model(DynamicModule):
    def forward(
        self,
...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen2ForCausalLM: "transformers.Qwen2ForCausalLM"})
class _Qwen2ForCausalLM(DynamicModule):
    def forward(
        self,
...
def register_wrap_modules(hf_model: Optional[Qwen2ForCausalLM] = None):
    pass
