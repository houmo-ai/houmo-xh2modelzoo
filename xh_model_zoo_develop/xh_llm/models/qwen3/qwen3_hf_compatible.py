...
#
# File: qwen3_hf_compatible.py
# Description:
#   Qwen3 HuggingFace-compatible loader for xh2modelzoo (HOUMO glue).
#
...
T = TypeVar("T", bound=Qwen3ForCausalLM)


def get_empty_hf_model(hf_model_dir, device_map="cpu", **kwargs) -> Qwen3ForCausalLM:
    """
    仅仅加载模型结构,不初始化权重,不占用显存
    """
    config = AutoConfig.from_pretrained(hf_model_dir)
    with no_init_weights(), init_empty_weights():
        hf_model: Qwen3ForCausalLM = AutoModelForCausalLM.from_config(
            config,
...
class Qwen3LegacyHFCompatible(Qwen3ForCausalLM):
    # _prefill: bool = True
    # _past_seq_length: int = 0
    # _embed_tokens: Optional[torch.nn.Module] = None
    # _llm_model: Optional[Qwen3LegacyInference] = None

    def setup(self, llm_model: Qwen3LegacyInference, embed_tokens: torch.nn.Module):
        self._prefill = False
    ...
    def to_hf_compatible(
        ...
        llm_model: Optional[Qwen3LegacyInference] = None,
    ) -> Qwen3ForCausalLM:
        """
        ...
        if llm_model is not None:
            assert isinstance(llm_model, Qwen3LegacyInference)
            hf_model.__class__ = cls
            assert isinstance(hf_model, Qwen3LegacyHFCompatible)
    ...
    def generate(self, *args: Any, **kwargs: Any) -> torch.LongTensor:
        ...
        class ForwardContext:
            def __init__(self, model: Qwen3LegacyHFCompatible):
                self._model = model
