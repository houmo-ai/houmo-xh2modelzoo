...
from xhquant.utils import fileio, is_list_of, is_seq_of, is_str, is_tuple_of

from ...image import bgr2hsv, clahe, hsv2bgr, imflip, imfrombytes, impad, imrescale, imresize, imrotate
from ...image.geometric import _scale_size, imresize_to_multiple
from ...image.photometric import imnormalize
from ...registry import TRANSFORMS
from ...structures.bbox import autocast_box_type, get_box_type
...
class BaseResize(BaseTransform):
    ...
    def __init__(
        ...
        assert scale is not None or scale_factor is not None, "`scale` and" "`scale_factor` can not both be `None`"
        if scale is None:
            self.scale = None
        else:
        ...
        if scale_factor is None:
            ...
            raise TypeError(f"expect scale_factor is float or Tuple(float), but" f"get {type(scale_factor)}")
    ...
    def _resize_bboxes(self, results: dict) -> None:
        ...
        if results.get("gt_bboxes", None) is not None:
            bboxes = results["gt_bboxes"] * np.tile(np.array(results["scale_factor"]), 2)
...
class ResizeToMultiple(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        for key in results.get("seg_fields", []):
            gt_seg = results[key]
            gt_seg = imresize_to_multiple(gt_seg, self.size_divisor, scale_factor=1, interpolation="nearest")
            results[key] = gt_seg
...
class AdjustGamma(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        results["img"] = lut_transform(np.array(results["img"], dtype=np.uint8), self.table)
    ...
    def transform(self, results: dict) -> dict:
        ...
        for key in results.get("seg_fields", []):
            if self.scale_factor != 1:
                results[key] = imrescale(results[key], self.scale_factor, interpolation="nearest")
        return results
...
class RandomRotFlip(BaseTransform):
    ...
    def random_rotate(self, results: dict) -> dict:
        ...
        results["img"] = imrotate(results["img"], angle=angle)
        for key in results.get("seg_fields", []):
            results[key] = imrotate(results[key], angle=angle)
        return results
...
class BaseRandomFlip(BaseTransform):
    ...
    def _flip_bbox(self, bboxes: np.ndarray, img_shape: Tuple[int, int], direction: str) -> np.ndarray:
        ...
        if self.swap_seg_labels is not None:
            ...
            for pair in self.swap_seg_labels:
                assert isinstance(pair, (tuple, list)) and len(pair) == 2, (
                    "swap_seg_labels must be a sequence with pair, but got " f"{self.swap_seg_labels}."
                )
    ...
    def _flip(self, results: dict) -> None:
        ...
        # flip bboxes
        if results.get("gt_bboxes", None) is not None:
            results["gt_bboxes"] = self._flip_bbox(results["gt_bboxes"], img_shape, results["flip_direction"])
...
@TRANSFORMS.register_module()
class RandomFlip(BaseRandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        for key in results.get("seg_fields", []):
            if results.get(key, None) is not None:
                results[key] = self._flip_seg_map(results[key], direction=results["flip_direction"]).copy()
                results["swap_seg_labels"] = self.swap_seg_labels
...
class RandomResize(BaseTransform):
    ...
    def _random_scale(self) -> tuple:
        ...
        if is_tuple_of(self.scale, int):
            ...
            scale = self._random_sample_ratio(self.scale, self.ratio_range)  # type: ignore
        elif is_seq_of(self.scale, tuple):
            scale = self._random_sample(self.scale)  # type: ignore
        else:
            raise NotImplementedError("Do not support sampling function " f'for "{self.scale}"')
    ...
    def transform(self, results):
        ...
        if self.bgr_to_rgb:
            ...
            if self.additional_targets:
                for key, value in self.additional_targets.items():
                    if value == "image":
                        results[key] = cv2.cvtColor(results["image2"], cv2.COLOR_RGB2BGR)
    ...
    def _pad_seg(self, results: dict) -> None:
        ...
        if results.get("gt_seg_map", None) is not None:
            ...
            results["gt_seg_map"] = impad(
                results["gt_seg_map"], shape=results["pad_shape"][:2], pad_val=pad_val, padding_mode=self.padding_mode
            )
