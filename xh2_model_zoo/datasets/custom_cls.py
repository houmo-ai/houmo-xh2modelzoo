...
from xhquant.utils.fileio import BaseStorageBackend, LocalBackend, get_file_backend, list_from_file
...
def find_folders(root: str, backend: Optional[BaseStorageBackend] = None) -> Tuple[List[str], Dict[str, int]]:
    ...
    backend: LocalBackend = backend or get_file_backend(root, enable_singleton=True)
    folders = list(
