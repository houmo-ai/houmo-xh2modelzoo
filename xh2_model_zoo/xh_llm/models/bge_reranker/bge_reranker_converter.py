...
from transformers import AutoModel, BertModel, AutoModelForSequenceClassification
...
class BGERerankerConverterXH2a(HFTransfromersConverter):
    ...
    def load_hf_model(self, hf_model_dir: str, **kwargs) -> Any:
        ...
        if self.mode == "embedder":
            model: BertModel = AutoModel.from_pretrained(hf_model_dir, device_map="cpu", torch_dtype=torch.float16)
        elif self.mode == "reranker":
            model: BertModel = AutoModelForSequenceClassification.from_pretrained(hf_model_dir, device_map="cpu", torch_dtype=torch.float16)
