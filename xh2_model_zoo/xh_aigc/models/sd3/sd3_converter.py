...
class SD3Converter:
    ...
    def export_onnx_vae_e(cls, hf_model, convert_config: SD3ConvertConfig, output_onnx_file):
        logger = get_root_logger()
        legacy_onnx = True
        simplify_onnx = True
        export_model = hf_model.vae.encoder
        export_model.to(device="cpu", dtype=torch.float32)
        shape = [1, 16, int(convert_config.height), int(convert_config.width)]
        onnx_name = Path(output_onnx_file).stem

        with torch.no_grad():
            export_model.eval()
            onnx_input = torch.rand((1, 3, 512, 512), dtype=torch.float32)
            if Path(output_onnx_file).exists():
                pass
            else:
                logger.info(f"exporting model {onnx_name} to onnx")
    ...
    def _convert(self, work_dir: str):
        target_device = self.convert_config.quant_scheme.target_device
        logger = get_root_logger()
        ...
        vae_encoder_onnx_file = str(Path(onnx_dir) / f"vae_encoder.onnx")
        ...
        if Path(vae_encoder_onnx_file).exists():
            logger.warning(f"{vae_encoder_onnx_file} already exists")
        else:
            self.export_onnx_vae_e(pipe, convert_config, vae_encoder_onnx_file)
        ...
        # vae encoder ====================================
        vae_encoder_input = [
            torch.randn((1, 3, int(convert_config.height), int(convert_config.width)), dtype=torch.float32),
        ]
        vae_encoder_hmonnx_file = str(Path(hmonnx_dir) / f"vae_encoder_{target_device.name}.onnx")
        if Path(vae_encoder_hmonnx_file).exists():
            logger.warning(f"{vae_encoder_hmonnx_file} already exists")
        else:
            convert_onnx_to_hmonnx(
                vae_encoder_onnx_file,
                vae_encoder_input,
                target_device,
                vae_encoder_hmonnx_file,
                input_names=["sample"],
        ...
        if False:
            ...
            session = HMONNXGoldenInference(vae_encoder_hmonnx_file)
            session.to(device)
            session.save_golden = True
            session.golden_dir = "/data01/home/xuchen/xh2/xh2_model_zoo/work_dirs/stable-diffusion-3-medium-diffusers_XH2a_512x512/hmonnx/golden_w8a8"
            session.step = 0
            session(vae_encoder_input[0].half().to(device))
        ...
        meta_info: Dict[str, Any] = {
            ...
            "vae_encoder_hmonnx": str(Path(vae_encoder_hmonnx_file).relative_to(work_dir)),
        }
