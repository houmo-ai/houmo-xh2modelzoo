...
class SD3HFCompatible(StableDiffusion3Pipeline):
    ...
    def to_hf_compatible(
        cls,
        hf_model_or_path: Union[StableDiffusion3Pipeline, str, StableDiffusion3Img2ImgPipeline],
        ...
        vae_encoder: Optional[HMONNXInference] = None,
    ):
        if vae_encoder is not None:
            if isinstance(hf_model_or_path, StableDiffusion3Img2ImgPipeline):
        ...
        if vae_encoder is not None:
            wrap_vae_encoder = VAE_encoder(vae_encoder)
            del pipe.vae.encoder
            pipe.vae.encoder = wrap_vae_encoder
        ...
        # pipe._execution_device = "cuda" if torch.cuda.is_available() else "cpu"
