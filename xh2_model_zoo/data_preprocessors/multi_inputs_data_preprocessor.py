...
class MultiInputsDataPreprocessor(BaseDataPreprocessor):
    def __init__(
        self,
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        for i_key in input_keys:
            if i_key == "data_samples":
                continue
            i_inputs = self.cast_data(data[i_key])
            if isinstance(i_inputs, torch.Tensor):
                inputs = i_inputs
            else:
                inputs = torch.stack(i_inputs)

            results[i_key] = inputs
