...
class ImgDataPreprocessor(BaseDataPreprocessor):
    ...
    def __init__(
        ...
        super().__init__(data_field=data_field, non_blocking=non_blocking)
        ...
        if pad_size is not None:
            assert is_seq_of(pad_size, int) and len(pad_size) == 2
    ...
    def forward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            ...
            if self.pad_size is not None:
                ...
                data_batch = torch.stack(batch_tensors)
            elif self.resize_shape is not None:
                batch_tensors: List[Tensor] = []
                for _batch_input in batch_inputs:
                    _batch_input = transforms.Resize(self.resize_shape)(_batch_input)
                    batch_tensors.append(_batch_input)
                data_batch = torch.stack(batch_tensors)
            else:
                data_batch = stack_batch(batch_inputs, self.pad_size_divisor, self.pad_value)
        # Process data with `default_collate`.
            ...
            pad_h = target_h - h
            pad_w = target_w - w
            data_batch = F.pad(_batch_inputs, (0, pad_w, 0, pad_h), "constant", self.pad_value)
        else:
            raise TypeError(
