...
from ...image import imflip, impad, imrescale, imrotate, imshear, imtranslate, rescale_size
...
class BitmapMasks(BaseInstanceMasks):
    ...
    def rescale(self, scale, interpolation="nearest"):
        ...
        if len(self.masks) == 0:
            ...
            rescaled_masks = np.stack([imrescale(mask, scale, interpolation=interpolation) for mask in self.masks])
    ...
    def resize(self, out_shape, interpolation="nearest"):
        ...
        if len(self.masks) == 0:
            ...
            resized_masks = np.stack(
                [imresize(mask, out_shape[::-1], interpolation=interpolation) for mask in self.masks]
            )
    ...
    def flip(self, flip_direction="horizontal"):
        ...
        if len(self.masks) == 0:
            ...
            flipped_masks = np.stack([imflip(mask, direction=flip_direction) for mask in self.masks])
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        if len(self.masks) == 0:
            ...
            padded_masks = np.stack([impad(mask, shape=out_shape, pad_val=pad_val) for mask in self.masks])
    ...
    def rotate(
        ...
        Args:
            ...
            scale (int | float): Isotropic scale factor.
            border_value (int | float): Border value. Default 0 for masks.
            interpolation (str): Same as in :func:`imrotate`.
