    ...
    def __setattr__(self, name: str, value: Union[torch.Tensor, np.ndarray]):
        ...
        if name in ("_metainfo_fields", "_data_fields"):
            ...
            assert isinstance(value, (torch.Tensor, np.ndarray)), (
                f"Can not set {type(value)}, only support" f" {(torch.Tensor, np.ndarray)}"
            )
            ...
            if value.ndim == 2:
                value = value[None]
                warnings.warn("The shape of value will convert from " f"{value.shape[-2:]} to {value.shape}")
            super().__setattr__(name, value)
        ...
        if isinstance(item, tuple):
            ...
            for index, single_item in enumerate(item[::-1]):
                if isinstance(single_item, int):
                    ...
                    raise TypeError(
                        "The type of element in input must be int or slice, " f"but got {type(single_item)}"
                    )
