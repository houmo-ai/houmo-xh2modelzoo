...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="/data01/datasets/Qwen2-7B",
        help="HF 模型 ID 或本地路径，例如 mistralai/GptOss-8x7B-v0.1",
    )
    ...
    save_dir = args.output_dir or f"work_dirs/{model_name}-{args.bits}bit-{args.group_size}group"
    ...
    if args.calibration_file is None:
        calib_dataset = build_calib_dataset(tokenizer, nsamples=args.num_samples, seqlen=args.seqlen)
    else:
        # args.calibration_file 一定要以 .jsonl 结尾
        import json

        assert args.calibration_file.endswith(".jsonl"), "calibration_file must be a .jsonl file"
        calib_dataset = list()
        with open(args.calibration_file, encoding="utf-8") as file:
            for i, line in enumerate(file):
                if i < args.nsamples:
                    calib_dataset.append(json.loads(line)["text"])
                else:
                    break
