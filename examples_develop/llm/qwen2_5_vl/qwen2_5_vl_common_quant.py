...
#
# File: qwen2_5_vl_common_quant.py
# Description:
#   Qwen2.5-VL common quantization entry adapted for the xh2 model zoo (xh2modelzoo).
...
def demo(model, processor):
    ...
    from xh_model_zoo_develop.xh_llm.quantization.quarot import utils
...
def main():
    ...
    if not args.skip_quarot:
        ...
        if not os.path.exists(filename):
            from xh_model_zoo_develop.xh_llm.quantization.quarot.quantizer_utils import quarot
    ...
    if not args.skip_gptq:
        from xh_model_zoo_develop.xh_llm.quantization.quarot.quantizer_utils import gptq
