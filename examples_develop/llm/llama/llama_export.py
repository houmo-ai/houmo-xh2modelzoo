...
from xh_model_zoo_develop.utils import MemoryTracker, TimeProfiler
from xh_model_zoo_develop.xh_llm.models.llama.llama_converter import LlamaConverterConfig, LlamaConverter
...
def main(args):
    ...
    config = LlamaConverterConfig.from_dict_or_other(cfg.to_dict())
    ...
    with TimeProfiler("convert", logger), MemoryTracker("cuda:0", "convert", logger):
        # converter = LlamaConverter(hf_model_path, config)
        # converter.export(work_dir, generate_golden=args.generate_golden)
        LlamaConverter.convert_and_export(hf_model_path, config, str(work_dir))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="/data01/datasets/llama3-8b-hf",
    )
