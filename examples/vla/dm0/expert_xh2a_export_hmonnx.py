...
#
# File: qwen3_legacy_xh2a_export_hmonnx.py
# Description:
#   Example script: llm/qwen3_legacy/qwen3_legacy_xh2a_export_hmonnx.py
#
...
from xh_model_zoo.xh_llm.models.dm0 import Qwen3LegacyConverterXH2a_Expert_config
...
def main(args):
    ...
    model_name = Path(hf_model_path).name + "_expert"
    target_device = DeviceType.XH2a
    quant_type = args.quant_type
    quant_scheme = QuantScheme(target_device=DeviceType.XH2a, quant_type=quant_type)
    # quant_scheme.nodes["lm_head"] = "w8a8h1_sefp"
    config = Qwen3LegacyConverterXH2a_Expert_config(
        batch_size=1,
    ...
    with TimeProfiler("convert", logger), MemoryTracker("cuda:0", "convert", logger):
        LLMConverter.from_pretrained(
            hf_model_path, "Qwen3ForCausalLM_legacy_DM0_Expert", config, str(work_dir)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="/data01/home/she.gao/vla/dexbotic/checkpoints/DM0-libero")
    parser.add_argument(
        "--context-length", type=int, default=3072, help="max sequence length"
    )
    parser.add_argument(
        "--input-sequence-length", type=int, default=50, help="input sequence length"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_logits_to_keep", type=int, default=0, help="not for test ppl"
    )
    parser.add_argument(
        "--quant-weight",
        type=str,
