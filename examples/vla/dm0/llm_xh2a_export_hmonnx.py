...
from xh_model_zoo.xh_llm.models.dm0 import Qwen3LegacyConverterXH2a_LLM_config
...
def main(args):
    ...
    config = Qwen3LegacyConverterXH2a_LLM_config(
        batch_size=1,
    ...
    with TimeProfiler("convert", logger), MemoryTracker("cuda:0", "convert", logger):
        LLMConverter.from_pretrained(
            hf_model_path, "Qwen3ForCausalLM_legacy_DM0", config, str(work_dir)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-sequence-length", type=int, default=2555, help="input sequence length"
    )
    parser.add_argument(
