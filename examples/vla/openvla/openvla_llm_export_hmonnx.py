...
from xh_model_zoo.xh_llm.models.openvla import XHLlamaModel
...
def export_onnx(xh_model, tokenizer, data_batch, out_dir, name, device, dtype):
    ...
    with torch.no_grad():
        ...
        token_id, token_str = decode_next_token(tokenizer, logits)
        logger.info(f"next token: {token_id} {token_str}")
