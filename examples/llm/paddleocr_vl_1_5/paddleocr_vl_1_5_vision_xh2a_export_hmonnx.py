...
if __package__:
    _common = import_module(f"{__package__}.common")
else:
    _common = import_module("common")
xhquant_llm_init = _common.xhquant_llm_init
get_root_logger = _common.get_root_logger
from xhquant.api import Config
from xh_model_zoo.xh_llm.models.builder import MODELS
from xh_model_zoo.xh_llm.models.paddleocr_vl_1_5 import PaddleOCRVLProcessor, XHPaddleOCRVLVisionModel
from xh_model_zoo.utils.memory_tracker import MemoryTracker
from xh_model_zoo.utils.time_profiler import TimeProfiler


def _preprocess_image_for_task(image: Image.Image, task: str):
...
def parse_arguments():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="xh2modelzoo/xh_model_zoo/xh_llm/models/paddleocr_vl_1_5/paddleocr_vl_1_5_vision_config.py",
    )
    parser.add_argument(
        "--hf_model_dir", type=str, default="/data01/datasets/PaddleOCR-VL-1.5"
    )
...
def _export_impl(cfg, args):
    ...
    paddleocr_vl_1_5_vision_model: XHPaddleOCRVLVisionModel = MODELS.build(cfg.model)
    native_model = paddleocr_vl_1_5_vision_model.get_hf_model()
    ...
    paddleocr_vl_1_5_vision_model.init_wrap_model(native_model)

    wraped_model = paddleocr_vl_1_5_vision_model._wrap_model
    # native_model.visual = wraped_model
    hf_model_dir = paddleocr_vl_1_5_vision_model.hf_model_dir
    ...
    # output full resized image so ONNX model handles patchification via Conv2d)
    from xh_model_zoo.xh_llm.models.paddleocr_vl.image_processing import SiglipImageProcessor

    processor.image_processor = SiglipImageProcessor.from_pretrained(hf_model_dir)
    ...
    image, max_pixels = _preprocess_image_for_task(image, args.task)
    messages = [
        {
            "role": "user",
            "content": [
    ...
    image_size_cfg = {"longest_edge": max_pixels}
    min_pixels = getattr(processor.image_processor, "min_pixels", None)
    if min_pixels is not None:
        image_size_cfg["shortest_edge"] = min_pixels
    ...
    inputs = processor.apply_chat_template(
        ...
        return_dict=True,
        return_tensors="pt",
        images_kwargs={"size": image_size_cfg},
    )
    ...
    hm_pixel_values = pixel_values.unsqueeze(0)
    ...
    hm_model.golden_dir = (
        Path(cfg.work_dir) / "vision" / f"paddleocr_vl_1_5_{model_type}_vision"
    )
