...
#
# File: mineru2.5_xh2a_hmonnx_demo.py
# Description:
#   Example script: llm/mineru2.5/mineru2.5_xh2a_hmonnx_demo.py
#
...
from xh_model_zoo.xh_llm.models.qwen2_vl.qwen2_vl_hf_compatible import Qwen2VL_HFCompatible 
...
def main(args):
            ...
            "content": [
                {
                    ...
                    "image": "data/images/houmo_logo.jpg",
                },
    ...
    xh_hf_model.data_preprocess = data_preprocess

    inputs_embeds, past_seq_length, _, position_ids, past_key_caches, past_value_caches = prefill_inputs
    prefill_session.past_key_caches = past_key_caches
    prefill_session.past_value_caches = past_value_caches
    
    output = xh_hf_model.generate(
        inputs["input_ids"].cuda(),
        inputs_embeds=inputs_embeds,
        max_length=8092,
        position_ids=position_ids.cuda(),
        prefill_length=input_seq_len,
    )

    output_ids = output.cpu().tolist()
    output_ids = [ids[len(in_ids) :] for in_ids, ids in zip(inputs.input_ids, output_ids)]
    output_ids = [[id for id in ids if id not in [151643, 151645]] for ids in output_ids]
    ...
    blocks = client.helper.parse_layout_output(out_texts[0])
    image = Image.open("data/images/houmo_logo.jpg")
    block_images, prompts, params, indices = client.helper.prepare_for_extract(image, blocks) # 1681x768 36
    ...
    resized_img = block_images[0].resize((1036, 1036), Image.Resampling.BICUBIC)
    inputs = processor(
        text=[text],
        images=resized_img,
        # videos=video_inputs,
