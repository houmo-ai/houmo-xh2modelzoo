#!/usr/bin/env python3
"""Categorized batch benchmark runner for Qwen3.5 spec-decode on hmonnx (xh2a)."""
...
def write_outputs(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    raw_json = out_dir / "mtp_demo_benchmark_raw.json"
    raw_csv = out_dir / "mtp_demo_benchmark_raw.csv"
    summary_json = out_dir / "mtp_demo_benchmark_summary.json"
    summary_md = out_dir / "mtp_demo_benchmark_summary.md"
    ...
    lines = [
        "# MTP Demo Benchmark Summary",
        "",
        "Latency uses `Qwen3_5SpecDecodeONNXModel.generate` wall-clock time on GPU, in milliseconds.",
        "",
        "| 类型 | 成功 | 失败 | avg asic decode model ms/run | fastest ms/run | slowest ms/run | avg ms/question | total calls | avg acceptance rate |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for item in summaries:
        lines.append(
            "| {category} | {ok} | {failed} | {weighted_asic_decode_model_ms_per_run:.3f} | "
            "{fastest_asic_decode_model_ms_per_run:.3f} | {slowest_asic_decode_model_ms_per_run:.3f} | "
            "{avg_asic_decode_model_total_ms_per_question:.3f} | {total_asic_decode_model_calls:.0f} | "
            "{avg_acceptance_rate:.2%} |".format(
                **item
            )
        )
...
def _build_row(
    *,
    ...
    if spec_mode == "dflash":
        ...
        decode_calls = int(result.get("mtp_decode_calls", 0))
...
def run_benchmark(args: argparse.Namespace) -> None:
    ...
    raw_json = args.out_dir / "mtp_demo_benchmark_raw.json"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Qwen3.5 xh2a hmonnx MTP demo benchmark (3 categories x N questions)"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        dest="out_dir",
        type=Path,
        default=Path("./tmp/mtp_demo_xh2a_benchmark"),
    )
