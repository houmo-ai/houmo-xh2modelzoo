...
def parse_arguments():
    ...
    parser.add_argument("--model_type", type=str, default="27B")
...
def _export_impl(cfg, args):
    ...
    golden_dir = Path(cfg.work_dir) / "vision" / f"qwen3_5_{args.model_type}_vision_{args.max_size_w}_{args.max_size_h}"
...
def main(args):
    ...
    cfg.work_dir = str(
        Path("./work_dirs")
        / f"qwen3_5_{args.model_type}"
        / f"{cfg_name}_{args.batch_size}_{args.max_size_t}_{args.max_size_h}_{args.max_size_w}"
          f"_use_gptq_model_{args.use_gptq_model}_{Path(args.hf_model_dir).stem}"
    )
