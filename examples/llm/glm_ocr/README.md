...
GLM-OCR 是一个基于视觉语言模型的 OCR 模型，支持图片文字识别。本目录包含完整的浮点推理、模型导出（Vision + LLM）、HMONNX 量化推理、Golden 数据导出以及 OCRBench 评测等脚本。
...
│   ├── img2.png
│   └── img3.png
├── glm_ocr_fp_demo.py                     # 1. 浮点推理 demo
├── glm_ocr_hmonnx_vision_xh2a_export.py   # 2. Vision 模块导出
├── glm_ocr_hmonnx_llm_xh2a_export.py      # 3. LLM 模块导出
├── glm_ocr_onnx_demo.py                   # 4. ONNX Vision + HF LLM 推理
├── glm_ocr_hmonnx_demo.py                 # 5. HMONNX 全量化推理 demo
├── glm_ocr_hmonnx_golden.py               # 6. Golden 参考数据导出
├── glm_ocr_ocrbench_fp_eval.py            # 7. OCRBench 浮点评测
├── glm_ocr_ocrbench_hmonnx_eval.py        # 8. OCRBench HMONNX 评测
└── run_ocrbench_compare.py                # 9. 评测结果对比
```
...
**默认模型路径**: `/data02/datasets/GLM-OCR/`（HuggingFace 格式）

## 脚本依赖关系

```
┌─────────────────────────┐
│  glm_ocr_fp_demo.py (1) │  ← 独立，仅需 HF 模型
└─────────────────────────┘

┌──────────────────────────────────────┐    ┌─────────────────────────────────────────┐
│  glm_ocr_hmonnx_vision_xh2a_export  │    │  glm_ocr_hmonnx_llm_xh2a_export.py (3)  │
│             .py (2)                  │    │                                         │
└───────────────┬──────────────────────┘    └──────────────────┬──────────────────────┘
                │                                              │
                └──────────────┬───────────────────────────────┘
                               ▼
              ┌────────────────────────────────┐
              │   glm_ocr_hmonnx_demo.py (5)   │
              │   glm_ocr_hmonnx_golden.py (6) │
              └────────────────────────────────┘

┌─────────────────────────────────┐     ┌─────────────────────────────────────┐
│ glm_ocr_ocrbench_fp_eval.py (7) │     │ glm_ocr_ocrbench_hmonnx_eval.py (8) │
│  (仅需 HF 模型)                 │     │  (需 2 + 3 的导出结果)               │
└───────────────┬─────────────────┘     └──────────────────┬──────────────────┘
                │                                          │
                └──────────────┬───────────────────────────┘
                               ▼
              ┌────────────────────────────────┐
              │   run_ocrbench_compare.py (9)  │
              └────────────────────────────────┘
```

## 脚本说明

### 1. 浮点推理 Demo (`glm_ocr_fp_demo.py`)

加载 HF 原始模型对单张图片进行浮点推理，用于验证模型基本功能。

```bash
python examples/llm/glm_ocr/glm_ocr_fp_demo.py \
...
### 3. LLM 模块导出 (`glm_ocr_hmonnx_llm_xh2a_export.py`)
...
### 4. ONNX Vision + HF LLM 推理 (`glm_ocr_onnx_demo.py`)

使用 ONNX Runtime 加载 Vision ONNX 模型，替换 HF 模型的 visual 部分，LLM 部分仍使用 HF 浮点权重进行推理。

```bash
python examples/llm/glm_ocr/glm_ocr_onnx_demo.py \
    --model /data02/datasets/GLM-OCR/ \
    --visual_onnx_path work_dirs/glm_ocr_onnx_export/glm_ocr_vision.onnx \
    --image examples/llm/glm_ocr/data/img3.png
```
...
|------|--------|------|
| `--model` | `/data02/datasets/GLM-OCR/` | HF 模型路径 |
| `--visual_onnx_path` | `work_dirs/glm_ocr_onnx_export/glm_ocr_vision.onnx` | Vision ONNX 路径 |
| `--images_json` | `None` | JSON 批量输入文件 |
| `--do_sample` | `False` | 是否采样解码 |

---

### 5. HMONNX 全量化推理 Demo (`glm_ocr_hmonnx_demo.py`)
...
| `--vision_mode` | `unquantized` | `quantized` / `unquantized` |
| `--use_fast` | `True` | HMONNX fast 模式 |
| `--images_json` | `None` | JSON 批量输入 |

---

### 6. Golden 参考数据导出 (`glm_ocr_hmonnx_golden.py`)
...
### 7. OCRBench 浮点评测 (`glm_ocr_ocrbench_fp_eval.py`)
...
### 8. OCRBench HMONNX 评测 (`glm_ocr_ocrbench_hmonnx_eval.py`)
...
### 9. 评测结果对比 (`run_ocrbench_compare.py`)
