...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Qwen3.5-MoE hmonnx inference demo",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--prompt", type=str, default="你好呀，你是谁，中文回答")
    ...
    parser.add_argument(
        ...
        type=str,
        default="fp16",
        choices=["fp16", "float16", "half", "fp32", "float32", "bf16", "bfloat16"],
    )
...
def main():
    ...
    print("Interactive Qwen3.5-MoE hmonnx chat, type '/exit' to quit.", flush=True)
    history = []
    while True:
        try:
            prompt = input("\nUser: ").strip()
