...
from xh_model_zoo.xh_llm.models.qwen3_next import XHQwen3NextModel


torch.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = False

DEFAULT_NO_SPLIT_MODULES = [
    "XHTrace_Qwen3NextDecoderLayer",
    "XHTrace_Qwen3NextSparseMoeBlock",
    "MoeBlock",
]
...
def _build_release_prefix(cfg) -> str:
    ...
    modelscope_name = str(release_cfg.get("modelscope_name", "qwen3next"))
...
def _export_single_graph(
    ...
    qwen3_next_model: XHQwen3NextModel = MODELS.build(model_cfg)
    logger.info(f"[{mode}] Loading HF model with device_map='auto'")
    native_model = qwen3_next_model.get_hf_model(
        device_map="auto",
    ...
    qwen3_next_model.init_wrap_model(native_model)
    _log_module_dtype(f"[{mode}] wrap model", qwen3_next_model.wrap_model, dtype, logger)
    del native_model
    ...
    _prepare_quanted_graph(qwen3_next_model, data_batch, cfg, dtype, logger)

    if getattr(args, "valid_exported", False):
        with torch.no_grad():
            outs = qwen3_next_model.test_step(data_batch)
...
def _prepare_export_context(cfg, args, logger):
    ...
    input_ids = _build_prompt_inputs(
        tokenizer,
        "cpu",
        qwen3_next_model.input_sequence_length,
        prompt=args.prompt,
    ...
    del native_model
    qwen3_next_model.to(dtype)
    _log_module_dtype("Wrap model", qwen3_next_model.wrap_model, dtype, logger)
    logger.info(
        f"Wrap cache dtype: conv={_format_dtype(_get_cache_dtype(getattr(qwen3_next_model, 'past_conv_caches', None)))}, "
        f"recurrent={_format_dtype(_get_cache_dtype(getattr(qwen3_next_model, 'past_recurrent_states', None)))}"
    )
    ...
    if qwen3_next_model.past_key_caches is not None and len(qwen3_next_model.past_key_caches) > 0:
        meta_info.kv_cache_shape = list(qwen3_next_model.past_key_caches[0].shape)
        meta_info.num_full_attention_layers = len(qwen3_next_model.past_key_caches)
    if (
        hasattr(qwen3_next_model, "past_conv_caches")
...
def main(args):
    ...
    if getattr(args, "golden_only", False) and getattr(args, "existing_work_dir", None):
        ...
        cfg.work_dir = str(Path("./work_dirs") / "qwen3_next_80b_a3b_instruct" / f"{cfg_name}_{Path(args.hf_model_dir).stem}")
...
def parse_arguments():
    ...
    parser.add_argument("--config", type=str, default="configs/qwen3_next/qwen3_next_80b_a3b_instruct_xh2a.py")
    parser.add_argument("--hf_model_dir", type=str, default="weights/Qwen3-Next-80B-A3B-Instruct")
    parser.add_argument(
        "--dtype",
        type=str,
        default="fp16",
