...
def replace_single_matmul_integer(model, graph, initializers, nodes, input_to_nodes, output_to_node):
    """
    ...
    if int8_input_tensor in output_to_node:
        ...
        if dq_node.op_type != "DynamicQuantizeLinear":
            ...
            for inp in dq_node.input:
                if inp in output_to_node and output_to_node[inp].op_type == "DynamicQuantizeLinear":
                    dq_node = output_to_node[inp]
                    break
    
    if not dq_node or dq_node.op_type != "DynamicQuantizeLinear":
        raise ValueError(f"未找到MatMulInteger[{matmul_int_node.name}]对应的DynamicQuantizeLinear")
    ...
    if int8_weight_name not in initializers:
        raise ValueError(f"MatMulInteger[{matmul_int_node.name}]的权重不是初始值，暂不支持动态权重")
...
def batch_dequantize_matmul(quant_onnx_path, output_onnx_path):
    ...
    quant_ops = [n.op_type for n in final_nodes if n.op_type in ["MatMulInteger", "DynamicQuantizeLinear"]]
    if quant_ops:
        print(f"警告：仍残留量化算子 {set(quant_ops)}，但已替换所有可处理的MatMulInteger")
    else:
        print("✅ 所有量化算子已替换完成")
    
    # 5. 保存最终FP32模型
    onnx.save(model_simp, output_onnx_path, save_as_external_data=True, size_threshold=1, all_tensors_to_one_file=True)
