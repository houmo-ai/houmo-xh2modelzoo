...
#
# File: minicpmo_tts_export_hmonnx.py
# Description:
#   Example script: llm/minicpmo/minicpmo_tts_export_hmonnx.py
#
...
def main(args):
    ...
    prefix = f"{model_name}-{target_device}-tts-{quant_type}"
    ...
    with TimeProfiler("convert", logger), MemoryTracker("cuda:0", "convert", logger):
        LLMConverter.from_pretrained(hf_model_path, "MiniCPMOTTS", config, str(work_dir))
