...
# File: hm_demo.py
# Description:
#   Example script: llm/whisper/hm_demo.py
#
...
def main(args):
    ...
    # 预分配 self-attention 的 K/V Cache（从模型输入 shape 自动读取 CACHE_MAX_LEN）
    CACHE_MAX_LEN = decoder.get_input(dec_names[6]).shape[2]
    k_cache = [CacheTensor(torch.zeros([1, num_heads, CACHE_MAX_LEN, head_dim], device=device, dtype=torch.float16)) for _ in range(num_decode_layers)]
    v_cache = [CacheTensor(torch.zeros([1, num_heads, CACHE_MAX_LEN, head_dim], device=device, dtype=torch.float16)) for _ in range(num_decode_layers)]
