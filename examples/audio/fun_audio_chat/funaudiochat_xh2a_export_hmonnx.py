...
from xh_model_zoo.xh_llm.models.funaudiochat._model import register_wrap_modules
from xh_model_zoo.xh_llm.models.funaudiochat._static_export import (
...
from xh_model_zoo.xh_llm.models.funaudiochat.constant import AUDIO_TEMPLATE
from xh_model_zoo.xh_llm.models.funaudiochat import Qwen3LegacyConverterXH2a
from xh_model_zoo.xh_llm.models.funaudiochat.register import register_funaudiochat
...
DEFAULT_AUDIO = "/data01/home/xuchen/xh2/xh2modelzoo/examples/audio/fun_audio_chat/asr_example_hotword.wav"
...
def build_padded_chunks(flat_input_features: torch.Tensor, chunk_lengths: list[int]) -> tuple[torch.Tensor, torch.Tensor]:
...
def run_decoder_prefill(
    ...
    if total_length <= chunk_length:
        past_seq_length = torch.zeros((decoder_text_embeds.shape[0],), dtype=torch.int32, device=decoder_text_embeds.device)
...
def preprocess_static_inputs(model, model_inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    if (not disable_text_branch) and "text_ids" in model_inputs:
        ...
        text_ids = torch.full((1, fixed_text_length), text_pad_value, dtype=torch.int32).to(model_inputs['input_features'].device)
        text_attention_mask = torch.zeros((1, fixed_text_length), dtype=torch.int32).to(model_inputs['input_features'].device)

    flat_input_features = model_inputs["input_features"].permute(0, 2, 1)[model_inputs["feature_attention_mask"].bool()].permute(1, 0).to(torch.float16)
    ...
    fixed_pooled_len = int(chunk_aftercnn_lens.max().item()) if int(chunk_aftercnn_lens.max().item()) < 2 else int(chunk_aftercnn_lens.max().item()) // 2
...
def build_dummy_inputs(model, processor, audio_path: str, prompt: str) -> Tuple[Dict[str, torch.Tensor], Dict[str, Any]]:
    import librosa
...
def validate_static_warp_consistency(model, split_inputs, logger):
    ...
    with torch.no_grad():
        ...
        continuous_audio_features, continuous_audio_output_lengths = model.get_audio_features(
            split_inputs["padded_input_features"][0, :, : int(split_inputs["chunk_lengths"][0])].unsqueeze(0).transpose(1, 2),
        ...
        audio_features, ref_audio_embeds, _ = model.audio_tower(
            ...
            continuous_audio_output_lengths=split_inputs["aftercnn_lens"].new_tensor(split_inputs["pooled_lengths"], dtype=torch.int32),
            feature_exist_mask=split_inputs["feature_exist_mask"],
        )
        ref_mixed_audio_features = (split_inputs["text_features"] + audio_features) / 2
        ref_audio_features = ref_mixed_audio_features * split_inputs["text_mix_mask"] + audio_features * (1.0 - split_inputs["text_mix_mask"])
        ref_merged_inputs_embeds = split_inputs["inputs_embeds"].clone()
        ref_merged_inputs_embeds[:, split_inputs["audio_token_positions"], :] = ref_audio_features[:, : len(split_inputs["pooled_lengths"]), :]
        ref_decoder_text_embeds = split_inputs["inputs_embeds"].clone()
        ref_decoder_text_embeds[:, split_inputs["audio_token_positions"], :] = split_inputs["text_features"][:, : len(split_inputs["pooled_lengths"]), :]
...
def export_audio_encoder(model, processor, args, work_dir: Path, logger) -> Path:
    ...
    (work_dir / "audio_encoder" / "meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
...
def export_audio_tower(model, args, work_dir: Path, logger) -> Path:
    ...
    (work_dir / "audio_tower" / "meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
...
def export_decoder(model, processor, args, work_dir: Path, logger) -> Path:
    ...
    prefill_out_file = work_dir / "audio_decoder" / "hmonnx" / f"funaudiochat_audio_decoder_prefill_{args.decoder_quant_type}.onnx"
    decode_out_file = work_dir / "audio_decoder" / "hmonnx" / f"funaudiochat_audio_decoder_decode_{args.decoder_quant_type}.onnx"
    prefill_out_file.parent.mkdir(parents=True, exist_ok=True)
    convert_quanted_model_to_hmonnx(quanted_prefill_model, prefill_input_args, str(prefill_out_file), prefill_input_names, prefill_output_names)
    ...
    convert_quanted_model_to_hmonnx(quanted_prefill_model, decode_input_args, str(decode_out_file), decode_input_names, decode_output_names)
    ...
    (work_dir / "audio_decoder" / "meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main(args):
    ...
    split_inputs, _ = build_dummy_inputs(model, processor, args.audio, args.system_prompt)
    # validate_static_warp_consistency(model, split_inputs, logger)

    register_wrap_modules(model)
    _ = wrap_llm_model(model.continuous_audio_tower, Config(dict()))
    _ = wrap_llm_model(model.audio_tower, Config(dict()))
    _ = wrap_llm_model(model.language_model, Config(dict(
    ...
    if getattr(model, "audio_invert_tower", None) is not None and hasattr(model.audio_invert_tower, "crq_transformer"):
        _ = wrap_llm_model(model.audio_invert_tower.crq_transformer, Config(dict(
            max_sequence_length=args.input_sequence_length * model.audio_invert_tower.group_size,
            input_sequence_length=args.input_sequence_length * model.audio_invert_tower.group_size,
            use_cache=True,
            num_logits_to_keep=0,
            kv_cache=dict(
                cache_axis=2,
    ...
    audio_tower_file = export_audio_tower(model, args, work_dir, logger)
    audio_file = export_audio_encoder(model, processor, args, work_dir, logger)

    export_meta = {
        "hf_model_path": hf_model_path,
        "audio_tower_hmonnx": str(audio_tower_file.relative_to(work_dir)),
        "audio_tower_meta": str((work_dir / "audio_tower" / "meta.json").relative_to(work_dir)),
        "audio_encoder_hmonnx": str(audio_file.relative_to(work_dir)),
        "audio_encoder_meta": str((work_dir / "audio_encoder" / "meta.json").relative_to(work_dir)),
        "context_length": args.context_length,
        "input_sequence_length": args.input_sequence_length,
        "audio_quant_type": args.audio_quant_type,
        "llm_quant_type": args.llm_quant_type,
        "decoder_quant_type": args.decoder_quant_type,
    }
    if args.export_only != "encoder":
        qwen_meta = export_qwen3(model, args, work_dir)
        decoder_prefill_file, decoder_decode_file = export_decoder(model, processor, args, work_dir, logger)
        export_meta.update(
            {
                "qwen3_meta": str(qwen_meta.relative_to(work_dir)),
                "audio_decoder_prefill_hmonnx": str(decoder_prefill_file.relative_to(work_dir)),
                "audio_decoder_decode_hmonnx": str(decoder_decode_file.relative_to(work_dir)),
                "audio_decoder_meta": str((work_dir / "audio_decoder" / "meta.json").relative_to(work_dir)),
            }
        )
    (work_dir / "export_meta.json").write_text(json.dumps(export_meta, ensure_ascii=False, indent=2), encoding="utf-8")
    logger.info("FunAudioChat split export finished")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default="/data01/datasets/Fun-Audio-Chat-8B")
    ...
    parser.add_argument("--audio-quant-type", type=str, default="w8a8h1_sefp")
    parser.add_argument("--llm-quant-type", type=str, default="w8a8h1_sefp")
    parser.add_argument("--decoder-quant-type", type=str, default="w8a8h1_sefp")
    parser.add_argument("--export-only", type=str, choices=["encoder", "all"], default="all")
