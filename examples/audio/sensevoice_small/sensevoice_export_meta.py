...
def rebuild_for_onnx(
    model: torch.nn.Module, max_seq_len: int = 512, device: str = "cpu", dynamic: bool = True
) -> torch.nn.Module:
    from funasr.utils.torch_function import sequence_mask

    model.device = device
    model.onnx_config = {"max_seq_len": max_seq_len, "dynamic": dynamic}
...
def _export_dummy_inputs(self: Any):
    if not self.onnx_config["dynamic"]:
        # Static shape: [1, max_seq_len, 560]
...
def _export_dynamic_axes(self: Any):
    if not self.onnx_config["dynamic"]:
        return None
