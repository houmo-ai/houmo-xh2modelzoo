...
# File: demo_new.py
# Description:
#   CosyVoice3 HMONNX runtime demo glue script (HOUMO inference pipeline).
#
...
def llm_inference(args, model_input):
    ...
    if 'prompt_text' not in model_input:
        ...
        prompt_text_len = model_input.get('prompt_text_len', 
                                         torch.tensor([prompt_text.shape[1]], dtype=torch.int32))
    ...
    text = hf_model._llm_model.token_embedding(text)
    sos_eos_emb = torch.load(args.sos_eos_emb)
    task_id_emb = torch.load(args.task_id_emb)
    prompt_speech_token_emb = hf_model._llm_model.speech_embedding(prompt_speech_token)
...
def main(args):
    ...
    # test = load_dataset("urarik/thchs30", split="test")
    ...
    for j in range(40,41):
        prompt_speech_16k = load_wav(args.prompt_speech_16k, 16000)
        prompt_text = 'You are a helpful assistant.<|endofprompt|>希望你以后能够做的比我还好呦。'
        save_dir = ".generate"
        os.makedirs(save_dir, exist_ok=True)  # 如果目录不存在就创建
        file_path = os.path.join(save_dir, f"zero_shot_{j}_demo.wav")
        # prompt_speech_16k = load_wav_new(test[j]['audio']["array"], 16000, test[j]['audio']['sampling_rate'])
        # prompt_text = test[j]['sentence']
        ...
        for i in tqdm(frontend.text_normalize(tts_text, split=True, text_frontend=True)):
            ...
            model_input = frontend.frontend_zero_shot(i, prompt_text, prompt_speech_16k, configs['sample_rate'])
            ...
            import ipdb;ipdb.set_trace()
            # this_tts_speech_token_un = torch.load("/data01/home/she.gao/CosyVoice/tts_speech_token.pt")
        ...
        torchaudio.save(file_path, this_tts_speech, 24000)
