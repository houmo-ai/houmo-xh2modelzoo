"""
CosyVoice3 Flow 模块加载器
============================

通过 hyperpyyaml 加载 CausalMaskedDiffWithDiT (DiT decoder)，提供 QAT
训练所需的模型加载和模块元信息。Flow 的 forward 已返回 {"loss": ...}，
无需额外 wrapper。
...
模块加载器模式（适配新模型时参考）:
    ...
    4. 加载时通过 overrides 禁用其他模块 (llm=None, hift=None)，
...
# Flow 的 forward(batch, device) 已返回 {"loss": ...}
# 不需要额外 wrapper
...
def load_flow_model(yaml_path: str, hf_model_dir: str):
    """通过 hyperpyyaml 加载 CausalMaskedDiffWithDiT。
    ...
    Returns
    -------
    nn.Module — CausalMaskedDiffWithDiT 实例
    """
