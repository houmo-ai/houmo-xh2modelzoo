...
def _export_impl(cfg: Config, args: argparse.Namespace):
    ...
    # 语音token的token_embedding和text_embedding也是模型的一部分，需要保存下来以供后续推理使用
    ...
    if False:
        ...
        xh_model.to(dtype)
        hf_compatible_model = build_qwen3_tts_talker_hf_compatible(hf_model, xh_model)
        # hf_compatible_model.dynamic_input = True
        hf_compatible_model.model.to(device=cfg.exec_device, dtype=dtype)
        wavs, sr = hf_compatible_model.generate_custom_voice(
            text="基于先进的存算一体技术和存储工艺，后摩智能致力于突破芯片的性能与功耗瓶颈，加速人工智能技术的普惠落地",
            language="Chinese",
            speaker="vivian",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./config/llm/qwen3_tts_12hz_0_6B_customvoice_talker_2k_xh2a.py",
    )
