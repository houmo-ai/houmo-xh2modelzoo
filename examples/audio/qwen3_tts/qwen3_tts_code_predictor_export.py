...
def _run_generate(hf_model, cfg):
    ...
    if mode == "voice_design":
        ...
        assert Path(ref_audio).exists(), (
            f"missing reference audio {ref_audio}; download clone_1.wav first"
        )
...
def _export_impl(cfg: Config, args: argparse.Namespace):
    ...
    xh_model.release_exported_model()  # clear exported model to avoid affecting later exports
    ...
    if args.golden:
        ...
        logger.info("*************** Finished exporting prefill golden ***************")
    ...
    if args.golden:
        ...
        logger.info("*************** Start exporting decode golden ***************")
        xh_model.reset_kvcache()
        ...
        logger.info("*************** Finished exporting decode golden ***************")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./config/llm/qwen3_tts_12hz_code_predictor_2k_xh2a.py",
        help="unified code_predictor component config; pick variant with --variant",
    )
