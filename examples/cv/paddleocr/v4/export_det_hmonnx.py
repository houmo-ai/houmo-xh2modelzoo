...
#
# File: export_det_hmonnx.py
# Description:
#   Example script: cv/paddleocr/v4/export_det_hmonnx.py
#
...
def main(args):
    ...
    if not os.path.exists(out_hmonnx_file):
        ...
        convert_onnx_to_hmonnx(
            onnx_file,
            [
                torch.randn(1, 3, 736, 736, dtype=torch.float32),
            ],
            DeviceType.XH2a,
            out_hmonnx_file,
            quant_config=quant_config,
            input_names=["generated_tensor_875"],
    ...
    x = np.load("/data01/home/yujy/work/OnnxOCR/det_input.npy")
    y = hm_model(torch.from_numpy(x).cuda().half())


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--onnx", type=str, default="data/models/paddleocr/v4/paddleocrv4_det-sim.onnx")
