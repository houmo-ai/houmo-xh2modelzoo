...
def main(args):
    ...
    convert_onnx_to_hmonnx(
        onnx_file,
        [torch.randn(422, 39, dtype=torch.float32)],
        DeviceType.XH2a,
        out_hmonnx_file,
        quant_config=quant_config,
        input_names=["new_indices_input"],
        # output_names=["outs"],
    )
    ...
    session.step = 0
    session(torch.randn(422, 39, dtype=torch.float16).to(device))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--onnx",
        type=str,
        default="/data01/home/xuchen/xh2/xh2_model_zoo/data/models/model-chenjunbo-shape-md.sim.onnx",
    )
