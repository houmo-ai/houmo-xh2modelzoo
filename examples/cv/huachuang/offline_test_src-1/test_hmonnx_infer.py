...
if __name__ == "__main__":
    ...
    encoder_hmonnx_path = "work_dirs/encoder_sim/hmonnx/encoder_sim_w8a8_sefp_XH2a.onnx"
    encoder = hmonnx_load_model(encoder_hmonnx_path)
    predictor = hi_load_model(model_dir, model_name="predictor")
    decoder_hmonnx_path = "work_dirs/decoder_sim/hmonnx/decoder_sim_w8a8_XH2a.onnx"
    decoder = hmonnx_load_model(decoder_hmonnx_path)
    ...
    pre_token_mask = np.zeros((1, 100), dtype=np.int32)
    ...
    pre_token_mask = torch.from_numpy(pre_token_mask)
    decoder_out = decoder(enc, enc_mask, pre_acoustic_embeds, pre_token_mask)       
    decoder_out = decoder_out[:, :pre_token_length.item()].float().detach().cpu().numpy()
