# -*- coding: utf-8 -*-
# Copyright 2024 Alibaba Inc. (FunASR Authors). All rights reserved.
...
#
# Source: https://github.com/modelscope/FunASR/blob/main/runtime/python/onnxruntime/funasr_onnx/utils/utils.py
# See licenses/funasr/LICENSE for the upstream Apache-2.0 license text.
#
# File: utils.py
# Description:
#   FunASR ONNX runtime utility helpers re-used by the Huachuang offline OCR test pipeline.
...
class OrtInferSession:
    ...
    def get_input_types(self):
        """Get the data types of model inputs"""
        return [v.type for v in self.session.get_inputs()]

    def get_input_dtype(self, input_name=None):
        """Get the numpy dtype for a specific input or the first input"""
        inputs = self.session.get_inputs()
        if input_name is None:
            # Return dtype of first input
        ...
        if 'float16' in input_type:
            ...
            # Default to float32 if type is not recognized
            return np.float32
...
def code_mix_split_words_jieba():
    try:
