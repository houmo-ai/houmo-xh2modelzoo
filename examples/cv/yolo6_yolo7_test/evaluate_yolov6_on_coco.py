...
#
# File: evaluate_yolov6_on_coco.py
# Description:
#   Example script: cv/yolo6_yolo7_test/evaluate_yolov6_on_coco.py
#
...
def postprocess_yolov7(outputs: Tuple[torch.Tensor], metas: Dict[str, Any]):
    ...
    for i, (out, stride, anchor) in enumerate(zip(outputs, strides, anchors)):
        ...
        out = out.view(bs, 3, 85, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
...
def postprocess_yolov6(outputs: torch.Tensor, metas: Dict[str, Any]):
    return _non_max_suppression(outputs, conf_thres=0.001, iou_thres=0.65)
...
def evaluate(opt):
    ...
    logger.info(f"正在加载模型: {opt.hmonnx}")
    xhquant_init(None)
    ...
    if opt.model_type == "yolov7":
        postprocess_fn = postprocess_yolov7
        logger.info("已选择 YOLOv7 后处理模式。")
    elif opt.model_type == "yolov6":
        postprocess_fn = postprocess_yolov6
        logger.info("已选择 YOLOv6 后处理模式。")
    else:
        raise ValueError(f"不支持的模型类型: {opt.model_type}. 请选择 'yolov6' 或 'yolov7'。")
    ...
    for batch in tqdm(dataloader):
        ...
        if isinstance(raw_outputs, tuple):
            ...
            outputs_cpu = raw_outputs.cpu()

        detections = postprocess_fn(outputs_cpu, metas)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hmonnx",
        type=str,
        default="work_dirs/yolov6m/hmonnx/yolov6m_w8a8h1_sefp_XH2a.onnx",
        help="量化后的 .hmonnx 模型文件路径",
    )
    parser.add_argument(
        "--model-type", type=str, default="yolov6", choices=["yolov6", "yolov7"], help="模型类型 (yolov6 或 yolov7)"
    )
    parser.add_argument(
        "--data", type=str, default="examples/cv/yolo6_yolo7_test/coco_eval.yaml", help="数据集配置文件路径"
    )
    ...
    # 新增参数以支持并行
    parser.add_argument("--output-json", type=str, default="coco_results.json", help="保存COCO格式JSON结果的路径")
