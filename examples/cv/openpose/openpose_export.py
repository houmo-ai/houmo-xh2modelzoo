...
#
# File: openpose_export.py
# Description:
#   Example script: cv/openpose/openpose_export.py
#
...
def main(args):
    ...
    # 步骤 0: 转换 Opset 版本，与 YOLOv5 脚本保持一致
    # 您的工具链可能需要特定的 Opset 版本，这里保留为 13
    convert_onnx_opset(onnx_file, target_opset=13)

    # 1. 准备输出路径 (逻辑不变，会自动适应 'openpose_body' 名称)
    ...
    # 3. 创建与 OpenPose 模型批次大小和尺寸匹配的随机输入
    input_shape = [batch_size, 3, 184, 128]
    print(f"Creating a random input tensor with shape: {input_shape}")
    dummy_input = torch.randn(*input_shape, dtype=torch.float32)

    # 4. --- API 调用 ---
    input_names = ["input"]
    print(f"--- [步骤 1] 开始使用 xhquanttool 转换模型 ---")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Export OpenPose ONNX to HMONNX with specified quantization.")
    ...
    parser.add_argument("--quant-type", default="w8a8-sefp", help="Quantization type (e.g. w8a8-sefp).")
