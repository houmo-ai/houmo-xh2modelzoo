...
def test_vit(trace_method: Literal["FX", "DFX", "ONNX"], w_bit: int, a_bit: int):
    import torch
    ...
    onnx_file = "data/model_zoo2/houmo/vit/vit_small_patch16_224_extract.onnx"
    ...
    image = cv2.imread("data/images/qwen2_vl_demo.jpeg")
    image = cv2.resize(image, (224, 224), interpolation=cv2.INTER_LINEAR) / 255.0
    input = torch.from_numpy(image).to(device).reshape(input_shape)
    ...
    outputs = session_onnx.run(None, {input_name:input.float().cpu().numpy()})
