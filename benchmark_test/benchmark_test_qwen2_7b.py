...
def test_qwen2(trace_method: Literal["FX", "DFX", "ONNX"], w_bit: int, a_bit: int):
    ...
    model_file = os.path.join(current_dir, "examples/llm/qwen2_legacy/qwen2_legacy_xh2a_export_hmonnx.py")
    ...
    if w_bit < 8:
        ...
        command = [
            ...
            "--model", "/data01/datasets/qwen2-7b-instruct",
            "--context-length", "2048",
    ...
    # ========================== eval model ==========================
    model_file = os.path.join(current_dir, "examples/llm/qwen2_legacy/qwen2_legacy_eval.py")
    meta_path = os.path.join(current_dir, "work_dirs/qwen2-7b-instruct-XH2a-2k-w8a8h1_sefp/meta.json")
    hf_model_path = os.path.join(current_dir, "work_dirs/qwen2-7b-instruct-XH2a-2k-w8a8h1_sefp/")
    eval_ppl_path = os.path.join(current_dir, "work_dirs/qwen2-7b-instruct-XH2a-2k-w8a8h1_sefp/eval_ppl.txt")
    command = [
        sys.executable,  # 使用当前Python解释器，避免环境不一致
        model_file,  # 注意：你原命令里多了一个.py后缀，需修正
        "--config", meta_path,
