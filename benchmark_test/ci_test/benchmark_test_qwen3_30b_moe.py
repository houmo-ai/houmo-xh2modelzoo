...
def test_qwen2(trace_method: Literal["FX", "DFX", "ONNX"], w_bit: int, a_bit: int):
    ...
    model_file = os.path.join(current_dir, "examples/llm/qwen3moe/qwen3_moe_xh2a_export_hmonnx.py")

    # ========================== 检查并下载模型 ==========================
    model_path = "work_dirs/fp_model/Qwen3-30B-A3B-1layer"
    download_url = "http://10.10.1.53:8082/artifactory/model_zoo2/model_zoo2/fp_models/qwen3_moe_1layer.tar.gz"
    ...
    if w_bit < 8:
        ...
        command = [
            ...
            "--model", "work_dirs/fp_model/Qwen3-30B-A3B-1layer",
            "--context-length", "2048",
    ...
    # ========================== eval model ==========================
    # model_file = os.path.join(current_dir, "examples/llm/qwen3moe/qwen3moe_eval.py")
    # meta_path = os.path.join(current_dir, "work_dirs/Qwen3-30B-A3B-XH2a-2k-w8a8h0_sefp/meta.json")
    # hf_model_path = os.path.join(current_dir, "work_dirs/Qwen3-30B-A3B-XH2a-2k-w8a8h0_sefp/")
    # eval_ppl_path = os.path.join(current_dir, "work_dirs/Qwen3-30B-A3B-XH2a-2k-w8a8h0_sefp/eval_ppl.txt")
    # command = [
    #     sys.executable,  # 使用当前Python解释器，避免环境不一致
    #     model_file,  # 注意：你原命令里多了一个.py后缀，需修正
    #     "--config", meta_path,
