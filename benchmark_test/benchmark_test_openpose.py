...
def test_openpose(trace_method: Literal["FX", "DFX", "ONNX"], w_bit: int, a_bit: int):
    ...
    onnx_file = "data/models/openpose_body.onnx"
    ...
    cos_sim = torch.cosine_similarity(hm_out[0], torch.from_numpy(outputs[0]).to(hm_out[0].device) ).mean()
...
if __name__ == "__main__":
    test_openpose("ONNX", w_bit=8, a_bit=16)
