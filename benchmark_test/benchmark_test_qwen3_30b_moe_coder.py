...
@allure.title("Qwen3-8B-moe-coder测试")
...
def test_qwen2(trace_method: Literal["FX", "DFX", "ONNX"], w_bit: int, a_bit: int):
    ...
    if w_bit < 8:
        ...
        command = [
            ...
            "--model", "/data02/datasets/Qwen3-Coder-30B-A3B-Instruct",
            "--context-length", "2048",
    ...
    meta_path = os.path.join(current_dir, "work_dirs/Qwen3-Coder-30B-A3B-Instruct-XH2a-2k-w8a8h0_sefp/meta.json")
    hf_model_path = os.path.join(current_dir, "work_dirs/Qwen3-Coder-30B-A3B-Instruct-XH2a-2k-w8a8h0_sefp/")
    eval_ppl_path = os.path.join(current_dir, "work_dirs/Qwen3-Coder-30B-A3B-Instruct-XH2a-2k-w8a8h0_sefp/eval_ppl.txt")
    command = [
        sys.executable,  # 使用当前Python解释器，避免环境不一致
        model_file,  # 注意：你原命令里多了一个.py后缀，需修正
        "--config", meta_path,
