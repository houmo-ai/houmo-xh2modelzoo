...
def test_emotion2vec_workflow_yaml_uses_audio_model_layout_and_w8a8():
    ...
    assert workflow_config.export["model"] == {
        "chip_arch": "XH2a",
        "model_type": "Emotion2vecForSequenceEmbedding",
        "hf_model": None,
...
def test_emotion2vec_workflow_model_config_round_trips(tmp_path):
    ...
    assert resolved.model_type == "Emotion2vecForSequenceEmbedding"
...
def test_emotion2vec_workflow_dump_golden_uses_standard_api(monkeypatch, tmp_path):
    ...
    export_dir.mkdir()
    meta = Emotion2vecModelMeta(window_samples=256000, feature_dim=1024)
    ...
    class FakeBridge(torch.nn.Module):
        def forward(self, waveform, valid_samples):
            ...
            assert valid_samples.dtype == torch.int32
            features = torch.arange(24, dtype=torch.float32).reshape(1, 3, 8)
            mask = torch.tensor([[False, False, True]])
            return features, mask

    monkeypatch.setattr(workflow_module, "load_funasr_emotion2vec_model", lambda path: object())
    monkeypatch.setattr(workflow_module, "Emotion2vecExportBridge", lambda model, window_samples: FakeBridge())
    ...
    assert np.load(export_dir / "golden/frame_features.npy").shape == (2, 8)
    assert np.load(export_dir / "golden/utterance_feature.npy").shape == (8,)
