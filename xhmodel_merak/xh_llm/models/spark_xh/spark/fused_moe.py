...
direct_register_custom_op(
    op_name="clamp_fused_experts",
    op_func=clamp_fused_experts,
    mutates_args=["hidden_states"],
    fake_impl=clamp_fused_experts_fake,
...
def fused_experts_impl(
    ...
    quant_dtype = _get_config_quant_dtype(
        use_fp8_w8a8=use_fp8_w8a8, use_int8_w8a8=use_int8_w8a8, use_mxfp4_w4a4=use_mxfp4_w4a4
    )
    ...
    for chunk in range((num_tokens // CHUNK_SIZE) + 1):
        ...
        if clamp_input_value > 0:
            curr_hidden_states = curr_hidden_states.clamp_(-clamp_input_value, clamp_input_value)
        ...
        if clamp_input_value > 0:
            intermediate_cache2 = intermediate_cache2.clamp_(-clamp_input_value, clamp_input_value)
        ...
        ops.moe_sum(
            intermediate_cache3.view(*intermediate_cache3.size()), out_hidden_states[begin_chunk_idx:end_chunk_idx]
        )

    return out_hidden_states


def torch_vllm_clamp_fused_experts(**kwargs) -> torch.Tensor:
    torch.ops.vllm.clamp_fused_experts(**kwargs)
...
def fused_experts(
    ...
    return torch_vllm_clamp_fused_experts(
