...
class Qwen35Workflow(BaseHMONNXWorkflow):
    ...
    def _quant_autoround_gptqmodel_hf(
        ...
        trust_remote_code = bool(runtime_cfg.get("trust_remote_code", quant_cfg.get("trust_remote_code", True)))
        iters = int(quant_cfg.get("iters", 200))
        sym = bool(quant_cfg.get("sym", True))

        dataset = self._format_autoround_dataset(calibration_cfg)
        # AutoRound names the GPTQ/GPTQModel-compatible HF exporter
        # "auto_gptq". Keep the public workflow artifact name stable as
        # "gptqmodel_hf" while using the upstream exporter identifier.
    ...
    def _format_autoround_dataset(calibration_cfg: Mapping[str, Any]) -> str:
        dataset = str(calibration_cfg.get("dataset", "wikitext"))
