...
HMONNXSession = Union[HMONNXGoldenInference, HMONNXGraphGoldenInference]
...
class Qwen3_5ONNXModel(nn.Module):
    @property
    def device(self):
        return self._device
    ...
    def set_exec_device(self, device) -> nn.Module:
        def apply_fn(module):
            if hasattr(module, "_set_exec_device"):
    ...
    def _apply_auto_offload(
        ...
        if isinstance(session, HMONNXGraphGoldenInference):
    ...
    def _create_hmonnx_session(self, onnx_path: str, session_name: str) -> HMONNXSession:
        if self._should_enable_cuda_graph(session_name):
            ...
            try:
                session = HMONNXGraphGoldenInference(
                    onnx_path,
                ...
                if "'str' object has no attribute 'name'" not in str(e):
                    raise
                session = HMONNXGoldenInference(onnx_path)
                session.initialize()
        self._set_hmonnx_session_golden_dir(session, Path(onnx_path).parent)
        if not self.auto_offload:
    ...
    @staticmethod
    def _set_hmonnx_session_golden_dir(session: HMONNXSession, golden_dir: Path) -> None:
        session.golden_dir = str(golden_dir)

    @staticmethod
    def _set_hmonnx_session_golden(
        session: HMONNXSession,
        enable: bool,
        *,
        reset_step: bool = True,
    ) -> None:
        session.save_golden = enable
        if reset_step and hasattr(session, "reset_step"):
            session.reset_step()
