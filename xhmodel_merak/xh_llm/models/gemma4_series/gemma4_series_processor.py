...
class Gemma4SeriesProcessorConfig(BaseConfig):
    def __init__(self):
        super().__init__()
...
class XHGemma4SeriesProcessor(Gemma4Processor):
    def __init__(
        self,
        ...
        *,
        video_max_patches: int | None = None,
        video_image_seq_length: int | None = None,
        video_pooling_kernel_size: int | None = None,
        **kwargs,
    ):
        super().__init__(
        ...
        self.config = Gemma4SeriesProcessorConfig()
        self.video_max_patches = video_max_patches
        self.video_image_seq_length = video_image_seq_length
        self.video_pooling_kernel_size = video_pooling_kernel_size

    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path: str, trust_remote_code: bool = True, **kwargs):
        video_max_patches = kwargs.pop("video_max_patches", None)
        video_image_seq_length = kwargs.pop("video_image_seq_length", None)
        video_pooling_kernel_size = kwargs.pop("video_pooling_kernel_size", None)
        ...
        if isinstance(processor, cls):
            ...
            processor.video_pooling_kernel_size = video_pooling_kernel_size
            return processor
        return cls(
            ...
            video_max_patches=video_max_patches,
            video_image_seq_length=video_image_seq_length,
            video_pooling_kernel_size=video_pooling_kernel_size,
    ...
    def __call__(self, *args, **kwargs):
        ...
        model_inputs = self._add_padded_video_inputs(model_inputs)
    ...
    def _audio_feature_mask_after_subsampling(input_features_mask: torch.Tensor) -> torch.Tensor:
        ...
        # Gemma4 audio has two stride-2 conv projection layers; HF updates the
        # valid-frame mask by slicing ``mask[:, ::2]`` after each layer.
    ...
    def _build_pooling_matrix(position_ids: torch.Tensor, valid_mask: torch.Tensor, kernel_size: int) -> torch.Tensor:
        """Build host-side [soft_tokens, patches] average-pooling matrix."""
    ...
    def _get_video_pooling_kernel_size(self) -> int:
        if self.video_pooling_kernel_size is not None:
            return int(self.video_pooling_kernel_size)
        image_processor = getattr(self, "image_processor", None)
        return int(getattr(image_processor, "pooling_kernel_size", 3))

    def _get_padded_video_patch_count(self, default: int) -> int:
        if self.video_max_patches is not None:
            return int(self.video_max_patches)
        if self.video_image_seq_length is not None:
            kernel_size = self._get_video_pooling_kernel_size()
            return int(self.video_image_seq_length) * kernel_size * kernel_size
        return int(default)

    def _add_padded_video_inputs(self, inputs):
        if "pixel_values_videos" not in inputs or "video_position_ids" not in inputs:
            return inputs

        pixel_values_videos = inputs["pixel_values_videos"]
        video_position_ids = inputs["video_position_ids"]
        target_patches = self._get_padded_video_patch_count(default=video_position_ids.shape[-2])
        current_patches = int(video_position_ids.shape[-2])
        if current_patches > target_patches:
            raise ValueError(
                "Gemma4 video VIT fixed input is smaller than processor output: "
                f"current_patches={current_patches}, target_patches={target_patches}. "
                "Increase video_visual_config.max_patches for this video contract."
            )
        if current_patches < target_patches:
            pad_len = target_patches - current_patches
            pixel_values_videos = torch.nn.functional.pad(pixel_values_videos, (0, 0, 0, pad_len))
            video_position_ids = torch.nn.functional.pad(video_position_ids, (0, 0, 0, pad_len), value=-1)
            inputs["pixel_values_videos"] = pixel_values_videos
            inputs["video_position_ids"] = video_position_ids

        if video_position_ids.dim() == 3:
            video_position_ids = video_position_ids.unsqueeze(0)
        original_shape = video_position_ids.shape[:-2]
        flat_position_ids = video_position_ids.reshape(-1, video_position_ids.shape[-2], video_position_ids.shape[-1])

        valid_mask = ~(flat_position_ids == -1).all(dim=-1)
        pixel_position_ids = flat_position_ids.clone()
        pixel_position_ids[~valid_mask] = 0

        kernel_size = self._get_video_pooling_kernel_size()
        pooling_matrix = self._build_pooling_matrix(flat_position_ids, valid_mask, kernel_size)
        video_soft_token_count = (valid_mask.sum(dim=-1) // (kernel_size * kernel_size)).to(torch.long)

        inputs["video_pixel_position_ids"] = pixel_position_ids.reshape(
            *original_shape,
            *pixel_position_ids.shape[-2:],
        ).to(torch.int32)
        inputs["video_pooling_matrix"] = pooling_matrix.reshape(*original_shape, *pooling_matrix.shape[-2:])
        inputs["video_visual_attention_mask"] = self._build_visual_attention_mask(valid_mask).reshape(
            *original_shape,
            1,
            1,
            valid_mask.shape[-1],
        )
        inputs["video_soft_token_count"] = video_soft_token_count.reshape(*original_shape)
        return inputs
...
def configure_gemma4_visual_processor(
    processor: XHGemma4SeriesProcessor,
    *,
    export_mode: str,
    max_size_w: int,
    max_size_h: int,
...
# Compatibility names used by existing Gemma4 callers.
Gemma4ProcessorConfig = Gemma4SeriesProcessorConfig
XHGemma4Processor = XHGemma4SeriesProcessor


__all__ = [
    "Gemma4ProcessorConfig",
    "Gemma4SeriesProcessorConfig",
    "XHGemma4Processor",
    "XHGemma4SeriesProcessor",
