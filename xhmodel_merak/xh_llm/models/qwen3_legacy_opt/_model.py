...
from xhquant.nn import FlashAttention, LLMCache, MaskedSoftmax, RMSNorm
...
class _Qwen3Attention(_Qwen3AttentionBase):
    ...
    def _setup(self, cfg: ConfigDict | dict[str, Any]):
        ...
        if self.use_flash_attention:
            self.flash_attn = FlashAttention(self.attn_hidden_dim, self.num_heads, batch_first=True, scale=self.scaling)
        else:
