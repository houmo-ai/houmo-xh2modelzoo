...
try:
    from transformers.models.qwen3.modeling_qwen3 import Qwen3SdpaAttention
except ImportError:
...
class RotaryMatrixCache(nn.Module):
    @staticmethod
    def build_rotary_matrix(cos, sin):
        seq_len, head_dim = cos.shape[-2:]
        ...
        weight[:, : head_dim // 2] += sin_diag[:, head_dim // 2 :]
        # weight = ((weight / (1/128)).round().clip(-128,127)) / 128 # 对其做一次量化

        return weight

    def __init__(self, rotary_emb: Qwen3RotaryEmbedding, max_seq_len=4096):
        super().__init__()
        device = next(rotary_emb.buffers()).device
        position_ids = torch.arange(0, max_seq_len).unsqueeze(0).to(device)
        cos, sin = rotary_emb(torch.randn(3).to(device), position_ids)
        rotatry_matrix = RotaryMatrixCache.build_rotary_matrix(cos, sin)
        self.register_buffer("rotary_matrix_buffer", rotatry_matrix)
        # self.rotary_matrix_buffer = nn.Parameter(rotatry_matrix, requires_grad=False)

    def forward(self, position_ids):
        return self.rotary_matrix_buffer[position_ids]


class _BaseRotaryEmbedding(DynamicModule):
...
class _BaseAttention(DynamicModule):
...
class _BaseDecoderLayer(DynamicModule):
    def forward(
        self,
        hidden_states: torch.Tensor,
...
class _BaseRMSNorm(DynamicModule):
...
class _BaseModel(DynamicModule):
    def forward(
        self,
    ...
    def _setup(self, cfg: ConfigDict):
        ...
        if hasattr(self.layers[0].self_attn, "rotary_emb"):
            ...
            raise AttributeError("Cannot locate rotary embedding module for Qwen3 model wrapper")

        return self


class _BaseForCausalLM(DynamicModule):
    def forward(
        self,
        ...
        if self.num_logits_to_keep == 0:
            ...
            # 取最后一个token的输出
            g_hidden_states = self.llm_gather(hidden_states, current_input_length - 1)
        logits = self.lm_head(g_hidden_states)

        if self.export_projector:
            hidden_states = self.chat_tts_projector(hidden_states)
    ...
    def _setup(self, cfg: ConfigDict):
        ...
        self.export_projector = False
        return self


def register_wrap_modules(hf_model: Optional[Qwen3ForCausalLM] = None):
    """Ensure wrapper classes are registered.

    The body is intentionally empty: the side effect lives in module import time,
    where the ``@XHLLM_TRACEABLE_MODULES.register_module`` decorators below run.
    ``minicpmo_llm_model.init_wrap_model`` lazily imports this module and calls
    this function, which is the only load point of this file. Removing the call
    would skip the LLM layer registration and break export.
    """
    pass


# ======================= Qwen3 registrations =======================
@XHLLM_TRACEABLE_MODULES.register_module({Qwen3RotaryEmbedding: "Qwen3RotaryEmbedding"})
class _Qwen3RotaryEmbedding(_BaseRotaryEmbedding):
    pass
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        ...
        Qwen3SdpaAttention: "Qwen3SdpaAttention",
    }
)
class _Qwen3Attention(_BaseAttention):
    """Qwen3Attention adds QK-norm (RMSNorm on q/k after projection, before RoPE)."""
    ...
    def forward(
        ...
        # Qwen3 QK-norm: apply RMSNorm on head_dim BEFORE transpose and RoPE
    ...
    def _setup(self, cfg: Union[ConfigDict, Dict[str, Any]]):
        ...
        # Wrap Qwen3's q_norm and k_norm (RMSNorm on head_dim) into xhquant RMSNorm
        if hasattr(self, "q_norm") and self.q_norm is not None:
            original_q_norm = self.q_norm
            q_norm_new = RMSNorm(
                self.head_dim,
                original_q_norm.variance_epsilon if hasattr(original_q_norm, "variance_epsilon") else 1e-6,
            )
            q_norm_new.weight = nn.Parameter(original_q_norm.weight.data.clone())
            self.q_norm = q_norm_new

        if hasattr(self, "k_norm") and self.k_norm is not None:
            original_k_norm = self.k_norm
            k_norm_new = RMSNorm(
                self.head_dim,
                original_k_norm.variance_epsilon if hasattr(original_k_norm, "variance_epsilon") else 1e-6,
            )
            k_norm_new.weight = nn.Parameter(original_k_norm.weight.data.clone())
            self.k_norm = k_norm_new

        return self


@XHLLM_TRACEABLE_MODULES.register_module({Qwen3DecoderLayer: "Qwen3DecoderLayer"})
class _Qwen3DecoderLayer(_BaseDecoderLayer):
    pass


@XHLLM_TRACEABLE_MODULES.register_module({Qwen3RMSNorm: "Qwen3RMSNorm"})
class _Qwen3RMSNorm(_BaseRMSNorm):
    pass


@XHLLM_TRACEABLE_MODULES.register_module({Qwen3Model: "Qwen3Model"})
class _Qwen3Model(_BaseModel):
    pass


@XHLLM_TRACEABLE_MODULES.register_module({Qwen3ForCausalLM: "transformers.Qwen3ForCausalLM"})
class _Qwen3ForCausalLM(_BaseForCausalLM):
    pass
