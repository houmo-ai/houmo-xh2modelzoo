...
def _prepare_4d_attention_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    raise ValueError(f"Unsupported attention mask dtype: {dtype}")


@register_llm_model("MiniCPMO45VisionModel", master=False)
class XHMiniCPMOVisionModel(XHMiniCPMOBaseModel):
    ...
    def init_wrap_model(self, hf_model=None):
        ...
        self.resampler = hf_model.resampler
        return self._init_wrap_model_with_llm_registry(vpm)
    ...
    def prepare_inputs(self, data: Dict[str, Union[torch.Tensor, Any]]) -> Any:
        ...
        if all_pixel_values:
            ...
            if tgt_sizes_t.dim() == 1:
                tgt_sizes_t = tgt_sizes_t.unsqueeze(0)
            elif tgt_sizes_t.dim() > 2:
                tgt_sizes_t = tgt_sizes_t.reshape(tgt_sizes_t.shape[0], -1)

            if tgt_sizes_t.shape[-1] < 2:
                tgt_sizes_t = tgt_sizes_t.repeat(1, 2)
            elif tgt_sizes_t.shape[-1] > 2:
                tgt_sizes_t = tgt_sizes_t[:, :2]
            ...
            raise ValueError("MiniCPM-o-4.5 Vision prepare_inputs requires pixel_values in data")
