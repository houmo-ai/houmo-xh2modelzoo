...
class BaseLLMHMONNXModel(HMONNXBaseModel):
    ...
    def set_page_attention_context(
        ...
        if isinstance(block_ids, torch.Tensor):
            block_ids_tensor = block_ids.to(device=self.device, dtype=torch.int32)
        else:
            block_ids_tensor = torch.tensor(block_ids, dtype=torch.int32, device=self.device)
        slot_mapping = slot_mapping.to(device=self.device, dtype=torch.int64)
