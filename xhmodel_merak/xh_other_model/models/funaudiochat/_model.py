...
def _patched_prepare_inputs_for_generation(
    ...
    if any(self.generate_speech) and speech_ids is not None and speech_ids.shape[-1] != 0:
        ...
        if (
            getattr(self, "use_hmonnx_audio_tower", False)
            and getattr(self, "hmonnx_audio_tower_session", None) is not None
        ):
            runtime_device = (
            ...
            audio_features = self.hmonnx_audio_tower_session(
                last_group_speech_ids.to(dtype=torch.int32, device=runtime_device)
            )
...
class FunAudioChatAudioEncoderExportWrapper(nn.Module):
    ...
    def forward(
        ...
        merged_inputs_embeds[:, self.audio_token_positions, :] = flat_audio_features.to(
...
class FunAudioChatHMONNXForConditionalGeneration(FunAudioChatForConditionalGeneration):
    ...
    def load_hmonnx_runtime(
        ...
        artifacts = export_meta.get("artifacts") or {}
        ...
        audio_encoder_rel = export_meta.get("audio_encoder_hmonnx") or artifacts.get("audio_encoder_hmonnx")
        if audio_encoder_rel:
            self.hmonnx_audio_encoder_session = HMONNXInference(str(work_dir / audio_encoder_rel))
        ...
        audio_tower_rel = export_meta.get("audio_tower_hmonnx") or artifacts.get("audio_tower_hmonnx")
        if audio_tower_rel:
            self.hmonnx_audio_tower_session = HMONNXInference(str(work_dir / audio_tower_rel))
        ...
        if qwen_meta_rel is None:
            qwen_meta_rel = (export_meta.get("artifacts") or {}).get("qwen3_meta")
        ...
        decoder_prefill_rel = export_meta.get("audio_decoder_prefill_hmonnx") or artifacts.get(
            "audio_decoder_prefill_hmonnx"
        )
        decoder_decode_rel = export_meta.get("audio_decoder_decode_hmonnx") or artifacts.get(
            "audio_decoder_decode_hmonnx"
        )
        if decoder_prefill_rel and decoder_decode_rel:
            self.hmonnx_decoder_prefill_session = HMONNXInference(str(work_dir / decoder_prefill_rel))
            ...
            self.hmonnx_decoder_decode_session = HMONNXInference(str(work_dir / decoder_decode_rel))
    ...
    def from_float_model(cls, model: FunAudioChatForConditionalGeneration):
        ...
        hmonnx_model.prepare_inputs_for_generation = types.MethodType(
            _patched_prepare_inputs_for_generation, hmonnx_model
        )
    ...
    def _audio_invert_tower_warp_forward(
        ...
        for step_idx in range(decoder.group_size - 1):
            ...
            step_logits = decoder_decode_wrapper(
                current_hidden,
                torch.full(
                    (bs,), decoder_prefill_valid_length + step_idx, dtype=torch.int32, device=hidden_states.device
                ),
    ...
    def _audio_invert_tower_hmonnx_forward(
        ...
        runtime_device = (
            self.hmonnx_decoder_prefill_session._device
            if hasattr(self.hmonnx_decoder_prefill_session, "_device")
            else torch.device("cuda:0")
        )
        ...
        if use_prefill:
            ...
            prefill_logits = self.hmonnx_decoder_prefill_session(
                *self._move_to_device(prefill_input_args, runtime_device)
            )
            prefill_logits = prefill_logits[:, :decoder_prefill_valid_length, :]
            crq_audio_tokens, prefill_logits = decoder.sampling_step(prefill_logits)
            decoder.crq_generate_tokens.append(crq_audio_tokens.unsqueeze(1))
...
def register_wrap_modules(hf_model=None):
    ...
    if hf_model is not None and hasattr(hf_model, "language_model"):
        ...
        if getattr(hf_model, "audio_invert_tower", None) is not None and hasattr(
            hf_model.audio_invert_tower, "crq_transformer"
        ):
