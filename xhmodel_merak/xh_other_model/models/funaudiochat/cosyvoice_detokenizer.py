...
submodule_path = os.environ.get("FUNAUDIOCHAT_COSYVOICE_ROOT")
import uuid
# 现在可以导入子模块中的库
...
def token2wav(cosyvoice3_model, tokens, embedding=None, token_hop_len=25 * 30, pre_lookahead_len=3):
    ...
    for token_segment in tokens_list:
        ...
        for i in range(0, len(token_segment), token_hop_len):
            ...
            this_speech = cosyvoice3_model.model.token2wav(
                this_token,
...
def get_audio_detokenizer(token_hop_len=25 * 30, model_path=None):
    submodule_path = os.environ.get("FUNAUDIOCHAT_COSYVOICE_ROOT")
    if not submodule_path:
        raise RuntimeError("Set FUNAUDIOCHAT_COSYVOICE_ROOT before using the CosyVoice detokenizer")
    sys.path.insert(0, submodule_path)
    sys.path.insert(0, os.path.join(submodule_path, "third_party", "Matcha-TTS"))
    from cosyvoice.cli.cosyvoice import CosyVoice3

    if model_path is None:
        model_path = os.environ.get("FUNAUDIOCHAT_COSYVOICE_MODEL")
    if not model_path:
        raise ValueError("model_path or FUNAUDIOCHAT_COSYVOICE_MODEL is required")
    logger.info(f"cosyvoice cuda: {torch.cuda.is_available()}")
    cosyvoice3 = CosyVoice3(model_path, load_trt=False, load_vllm=False, fp16=False)
    cosyvoice3.model.flow.decoder.estimator.static_chunk_size = 2 * token_hop_len
    logger.info(f"cosyvoice loaded")
    return cosyvoice3


def tts_infer_streaming(
    tts_model,
    tts_spk_embedding,
    tokens,
    offset,
