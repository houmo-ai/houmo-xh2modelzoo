...
from xhquant.nn import AdaRMSNorm, LLMCache, MaskedSoftmax, Rope
...
class _GemmaRMSNorm(DynamicModule):
    def forward(self, hidden_states, cond):
        modulation = self.dense(cond) #* scale
        modulation = modulation.unsqueeze(1)
        # scale, shift, gate = torch.chunk(modulation, 3, -1)
        scale, shift, gate = torch.split(modulation, [1024, 1024, 1024], dim=-1)
        # scale = torch.load("/data01/home/she.gao/xhquant_llm/examples/scale.pt")
        # hidden_states = torch.load("/data01/home/she.gao/xhquant_llm/examples/x.pt")
        weight = 1.0 + scale.to(torch.float32)
        output = self.norm(hidden_states, weight)
        output = output + shift.to(torch.float32)
        return output.to(torch.float32), gate

    def _setup(self, cfg: Optional[Dict] = None):
        # self.dense = nn.Linear(self.cond_dim, self.dim * 3, bias=True)
        hidden_size = self.dense.weight.shape[1]
        self.norm = AdaRMSNorm(hidden_size, self.eps)
        return self
...
class _GemmaModel(DynamicModule):
    def _setup_position_cached(self):
        ...
        self.register_buffer("sin_cached", sin.to(dtype=torch.float32), persistent=False)
        self.register_buffer("cos_cached", cos.to(dtype=torch.float32), persistent=False)
...
def register_wrap_cls(hf_model):
    ...
    registrations = (
        (type(hf_model), _GemmaForCausalLM),
        (type(model), _GemmaModel),
        (type(model.layers[0]), _GemmaDecoderLayer),
        (type(model.norm), _GemmaRMSNorm),
    )
