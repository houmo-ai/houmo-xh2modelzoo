...
# Description:
#   CosyVoice3 TTS LLM model (Qwen2 0.5B) for the xh2 model zoo (merak).
...
@register_other_model("XHCosyVoice3LLM")
class XHQwen2LegacyModel(LLMBaseModel):
    """CosyVoice3 Qwen2 0.5B LLM export model.

    Registered as the main export model (``export.model.type``) so that
    :class:`AutoOtherModelWorkflow` can resolve this class and bind
    :class:`CosyVoice3Workflow` via :attr:`WORKFLOW_CLS`.
    """

    WORKFLOW_CLS = "xhmodel_merak.xh_other_model.models.cosyvoice3.workflow:CosyVoice3Workflow"
    ...
    def init_wrap_model(self, hf_model=None):
        ...
        # token_embedding.weight and lm_head.weight share the same object
    ...
    def load_wraped_model_state_dict_prefix(self, native_model, checkpoint: str):
        """Load CosyVoice3 ``llm.pt`` checkpoint, stripping the ``llm.model.`` prefix."""
