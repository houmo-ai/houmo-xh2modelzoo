...
class TextDurationStatic(nn.Module):
    ...
    def __init__(self, model: nn.Module, text_max_length: int) -> None:
        ...
        self.text_lstm = PackedBidirectionalLSTMStatic(model.text_encoder.lstm, self.text_max_length)
    ...
    def forward(
        ...
        for block in self.duration_blocks:
            values = block(values, prosody_style, valid_len, text_mask)
        duration_features = values
        duration_logits = self.duration_predictor(duration_features, valid_len, text_mask)

        text_values = text_features.transpose(1, 2) * text_mask
        text_encoded = self.text_lstm(text_values, valid_len) * text_mask
...
class FrameCoreStatic(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.shared_lstm = PackedBidirectionalLSTMStatic(model.predictor.shared, self.frame_max_length)
    ...
    def forward_acoustic(
        ...
        shared = self.shared_lstm(shared_input, valid_frames).transpose(1, 2) * mask_f
        prosody_style = style[:, 128:]
    ...
    def forward_generator_from_sine(
        ...
        return self._generator_from_source(decoder_feature, harmonic_source, style, valid_frames)
...
class FrameSynthesisStatic(nn.Module):
    ...
    def forward(self, encoded: Tensor, asr: Tensor, style: Tensor, valid_frames: Tensor) -> Tensor:
        decoder_feature, f0 = self.core.forward_acoustic(encoded, asr, style, valid_frames)
        return self.core.forward_generator_from_f0(decoder_feature, f0, style, valid_frames)
...
class GeneratorISTFTStatic(nn.Module):
    ...
    def forward(
        ...
        return self.core.forward_generator_from_sine(decoder_feature, sine, f0, style, valid_frames)
...
def run_duration_alignment_host(
    ...
    *,
    ...
    if (
        text_encoded.ndim != 3
        or text_encoded.shape[0] != 1
        or text_encoded.shape[2] != duration.shape[1]
    ):
...
def export_text_bucket(
    *,
    ...
    artifact, _ = _export_partition(
        ...
        expected_lstm_nodes=5,
        npu_graph=True,
        lstm_reverse_indices=make_reverse_idx(text_max_length, sample.valid_len),
    )
...
def export_frame_bucket(
    *,
    ...
    reverse_indices = make_reverse_idx(frame_max_length, feed["valid_frames"])
    graphs: dict[str, GraphArtifact] = {}
    feeds: dict[str, dict[str, Tensor]] = {}
    outputs: dict[str, dict[str, Tensor]] = {}
    ...
    if phase_on_npu:
        ...
        with torch.no_grad():
            waveform = module(*tuple(feed.values()))
        ...
        artifact, _ = _export_partition(
            ...
            output_path=(
                ...
                / bucket_key
                / f"kokoro_frame_synthesis_b1_f{frame_max_length}.onnx"
            ),
            opset=opset,
            simplify=False,
            validate_onnx=validate_onnx,
            expected_lstm_nodes=1,
            npu_graph=True,
            lstm_reverse_indices=reverse_indices,
        )
        graphs[FRAME_SYNTHESIS_ROLE] = artifact
        feeds[FRAME_SYNTHESIS_ROLE] = {name: value.detach().cpu().contiguous() for name, value in feed.items()}
    ...
    acoustic_outputs = dict(
        zip(("decoder_feature", "f0", "phase_increments"), acoustic_values, strict=True)
    )
    acoustic_artifact, _ = _export_partition(
        ...
        output_path=(
            ...
            / FRAME_ACOUSTIC_ROLE
            / bucket_key
            / f"kokoro_frame_acoustic_b1_f{frame_max_length}.onnx"
        ),
        opset=opset,
        simplify=False,
        validate_onnx=validate_onnx,
    ...
    outputs[FRAME_ACOUSTIC_ROLE] = {
        name: value.detach().cpu().contiguous() for name, value in acoustic_outputs.items()
    }
    ...
    phase_artifact, _ = _export_partition(
        ...
        output_path=(
            ...
            / PHASE_CORE_ROLE
            / bucket_key
            / f"kokoro_phase_core_b1_f{frame_max_length}.onnx"
        ),
        opset=opset,
        simplify=False,
        validate_onnx=validate_onnx,
    ...
    generator_artifact, _ = _export_partition(
        ...
        output_path=(
            ...
            / bucket_key
            / f"kokoro_generator_istft_b1_f{frame_max_length}.onnx"
        ),
        opset=opset,
        simplify=False,
        validate_onnx=validate_onnx,
    ...
    feeds[GENERATOR_ISTFT_ROLE] = {
        name: value.detach().cpu().contiguous() for name, value in generator_feed.items()
    }
