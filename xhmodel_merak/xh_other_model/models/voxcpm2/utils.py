...
def resolve_export_device(device: str | torch.device | None = None) -> torch.device:
    """Resolve and validate the execution device for VoxCPM2 exporters.

    ``None`` preserves the standalone scripts' historical behavior: prefer
    CUDA when available and otherwise use CPU. Workflow callers pass their
    ``--device`` value explicitly.
    """
    ...
    try:
        ...
        raise ValueError(f"Invalid export device {requested!r}.") from exc

    if resolved.type not in {"cpu", "cuda"}:
        raise ValueError(
            "VoxCPM2 export device must be 'cpu', 'cuda', or 'cuda:N', "
            f"but got {requested!r}."
        )

    if resolved.type == "cuda":
        if not torch.cuda.is_available():
            raise RuntimeError(
                f"CUDA export device {requested!r} was requested, but CUDA is not available."
            )
        visible_count = torch.cuda.device_count()
        if resolved.index is not None and resolved.index >= visible_count:
            raise ValueError(
                f"CUDA export device index {resolved.index} is out of range; "
                f"only {visible_count} CUDA device(s) are visible."
            )

    return resolved


def activate_export_device(device: str | torch.device | None = None) -> torch.device:
    """Resolve the export device and make it the process' current CUDA device.

    Some third-party export/quantization code creates tensors on the unindexed
    ``"cuda"`` device. Selecting the requested CUDA device up front makes those
    allocations follow the same device as the explicitly placed model tensors.
    """
    resolved = resolve_export_device(device)
    if resolved.type == "cuda":
