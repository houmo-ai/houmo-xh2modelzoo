...
# File: modeling_t5.py
# Description:
#   Modeling T5 model implementation.
#
...
class T5DenseActDense(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # modify by zoujy2 support gptq and gptq inference
        ...
        elif hasattr(self.wo, "module") and hasattr(self.wo.module, "weight"):
            if (
                isinstance(self.wo.module.weight, torch.Tensor)
                and hidden_states.dtype != self.wo.module.weight.dtype
                and self.wo.module.weight.dtype != torch.int8
            ):
                hidden_states = hidden_states.to(self.wo.module.weight.dtype)
        elif hasattr(self.wo, "scales"):
            if (
                isinstance(self.wo.scales, torch.Tensor)
                and hidden_states.dtype != torch.float32
                and self.wo.scales.dtype != torch.int8
            ):
