...
class SD3LenovoDiffusion3Pipe(StableDiffusion3Pipeline):
    @classmethod
    def from_pretrained(
        cls,
        hf_model_or_path: str,
        lenovo_model_or_path: str,
        mmdit_quant: bool = True,
        t5_quant: bool = True,
    ):
        logger = get_root_logger()
        logger.info("loading lenovo model")
        dtype = torch.float16
        ...
        if mmdit_quant:
            ...
            filename = str(Path(lenovo_model_or_path) / "mmdit_hypersd__non_sym_8bit_v1.3.0.1.safetensors")
            state_dict = load_file(filename)
            logger.info(f"loaded quanted mmdit:{filename}")
        else:
            filename = str(Path(lenovo_model_or_path) / "mmdit_hypersd_fp16_v1.3.0.0.safetensors")
            state_dict = load_file(filename)
        ...
        if t5_quant:
            ...
            filename = str(Path(lenovo_model_or_path) / "T5_non_sym_2bit_v1.3.0.1.safetensors")
