...
def wrap_vae_model(vae_model: nn.Module) -> nn.Module:
    ...
    for _, module in list(vae_model.named_modules()):
        dynamic_cls = wrapped_classes.get(type(module).__name__)
        if dynamic_cls is not None and not isinstance(module, DynamicModule):
            dynamic_cls.convert(module)
            converted_count += 1
    logging.info("Wan2.2 VAE warp converted modules: %d", converted_count)
    # if converted_count == 0:
    #     raise RuntimeError("Wan2.2 VAE warp failed: no modules were converted")
    return vae_model
