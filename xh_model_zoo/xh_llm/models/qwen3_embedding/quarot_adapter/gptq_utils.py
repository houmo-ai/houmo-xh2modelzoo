...
# File: gptq_utils.py
# Description:
#   Qwen3 Embedding post-training quantization adapted for the xh2 model zoo (xh2modelzoo).
...
from xh_model_zoo.xh_llm.quarot import quant_utils, utils
    ...
    for k in cache_state_dict:
        ...
        if paths[-1] == "quant_weight":
            ...
            submodule.register_parameter(
                "quant_weight", nn.Parameter(cache_state_dict[k], requires_grad=False)
            )
...
def gptq_fwrd(
    ...
    dtype = (
        ...
        else next(iter(model.parameters())).dtype
    )
    ...
    if is_qwen2_5_vl or is_qwen3_vl:
        ...
        inps = torch.zeros(
            (nsamples, seqlen, model.config.hidden_size), dtype=dtype, device=ddevice
        )
            ...
            if is_qwen2_5_vl or is_qwen3_vl:
                ...
                if tokenizer is not None:
                    ...
                    if isinstance(texts, str):
                        texts = [texts]
                    if cache["i"] < 3:
                        print(f"calib text[{cache['i']}]: {texts[0]}")
                    inputs = tokenizer(
                        texts,
                        return_tensors="pt",
                        truncation=True,
                ...
                if tokenizer is not None:
                    ...
                    inputs["input_ids"] = F.pad(
                        inputs["input_ids"], (0, seqlen - inputs["input_ids"].shape[1])
                    )
    ...
    if is_qwen2_5_vl or is_qwen3_vl:
        ...
        model.model.language_model.rotary_emb = (
            model.model.language_model.rotary_emb.cpu()
        )
    ...
    for i in range(len(layers)):
        ...
        if layer_cache_file is not None and Path(layer_cache_file).exists():
            ...
            try:
                ...
                print(
                    f"mem: {mem_info.rss / 1024 / 1024 / 1024:.2f} GB",
                    flush=True,
                    end=" ",
                )
            except Exception:
        ...
        for names in sequential:
            ...
            for j in range(nsamples):
                if is_qwen2_5_vl:
                    ...
                    hs = inps[j].to(device=device, dtype=torch.float32)
                    if hs.dim() == 2:
                        hs = hs.unsqueeze(0)
                    pos_ids = position_ids[j].to(device)
                    if pos_ids.dim() == 1:
            ...
            for name in subset:
                ...
                if quant_value.max().item() < pow(
                    2, 7
                ) and quant_value.min().item() >= -pow(2, 7):
                    quant_value = quant_value.to(torch.int8)
                elif quant_value.max().item() < pow(
                    2, 15
                ) and quant_value.min().item() >= -pow(2, 15):
                    quant_value = quant_value.to(torch.int16)
                subset[name].register_parameter(
                    "quant_weight", nn.Parameter(quant_value, requires_grad=False)
                )
                gptq[name].free()
    ...
    if is_qwen3_embedding and hasattr(base_model, "post_norm_linear"):
        # Allow user to skip GPTQ for this layer (e.g., custom-added layer)
        is_inference = False
        if hasattr(base_model.post_norm_linear, "weight"):
            w = base_model.post_norm_linear.weight
            try:
                if hasattr(w, "is_inference"):
                    is_inference = w.is_inference() if callable(w.is_inference) else bool(w.is_inference)
            except Exception:
                is_inference = False

        if getattr(base_model.post_norm_linear, "skip_gptq", False) or is_inference:
            print(f"\npost_norm_linear: skipped", flush=True, end=" ")
        else:
            print(f"\npost_norm_linear:", flush=True, end=" ")
            base_model.post_norm_linear = base_model.post_norm_linear.to(
                device=device, dtype=torch.float32
            )
            gptq = {}
            name = "post_norm_linear"
            layer_weight_bits = w_bits
            layer_weight_sym = not (w_asym)
            gptq[name] = GPTQ(base_model.post_norm_linear)
            gptq[name].quantizer = quant_utils.WeightQuantizer()
            gptq[name].quantizer.configure(
            ...
            handles = []
            handles.append(
                base_model.post_norm_linear.register_forward_hook(add_batch(name))
            )
            for j in range(nsamples):
                hs = inps[j].to(device=device, dtype=torch.float32)
                if hs.dim() == 2:
                    hs = hs.unsqueeze(0)
                base_model.post_norm_linear(hs)
            ...
            quant_value = gptq[name].W_int
            if quant_value.max().item() < pow(2, 7) and quant_value.min().item() >= -pow(
                2, 7
            ):
                quant_value = quant_value.to(torch.int8)
            elif quant_value.max().item() < pow(2, 15) and quant_value.min().item() >= -pow(
                2, 15
            ):
                quant_value = quant_value.to(torch.int16)
            base_model.post_norm_linear.register_parameter(
                "quant_weight", nn.Parameter(quant_value, requires_grad=False)
            )
            gptq[name].free()
            base_model.post_norm_linear = base_model.post_norm_linear.cpu()
            del gptq
            torch.cuda.empty_cache()
        ...
        if head_layer_cache_file is not None and Path(head_layer_cache_file).exists():
            ...
            if layers_cache_dir is not None and head_layer_cache_file is not None:
                ...
                safetensors_save_file(
                    head_state_dict, str(head_layer_cache_file)
                )  # Ensure string path
            del gptq
            torch.cuda.empty_cache()
