...
# File: model_utils.py
# Description:
#   Qwen3 Embedding model utilities adapted for the xh2 model zoo (xh2modelzoo).
...
# from xh_model_zoo.xh_llm.models.sensevoice.yuanma.model import SenseVoiceSmall
...
QWEN3_BASE_MODEL = transformers.models.qwen3.modeling_qwen3.Qwen3Model
...
def model_type_extractor(model):
    if isinstance(model, LLAMA_MODEL):
        ...
        return QWEN3_MODEL
    elif isinstance(model, QWEN3_BASE_MODEL):
        return QWEN3_BASE_MODEL
...
def get_mlp_bottleneck_size(model):
    ...
    if (
        ...
        or model_type == QWEN3_BASE_MODEL
...
def get_process_memory_info():
    ...
    GB = 1024 * 1024 * 1024
    return {
        "rss": round(memory_info.rss / GB, 2),
        "vms": round(memory_info.vms / GB, 2),
    }  # 物理内存  # 虚拟内存
