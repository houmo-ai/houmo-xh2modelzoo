...
from xh_model_zoo.xh_llm.quarot import data_utils, utils

from . import gptq_utils, rotation_utils
...
def quarot(model, rotate_mode="hadamard", device=None, quarot_matrix_size=None):
    ...
    # fuse layernorm
    model_type = rotation_utils.model_utils.get_model_type(model)
    rotation_utils.fuse_layer_norms(model)

    # rotation
    rotation_utils.rotate_model(
        model, rotate_mode, device=device, quarot_matrix_size=quarot_matrix_size
    )
...
def gptq(
    ...
    if calib_dataset not in [
        ...
        gptq_utils.gptq_fwrd(
            ...
            is_qwen3_embedding=is_qwen3_embedding,
            processor=processor,
            tokenizer=tokenizer,
        )
    else:
