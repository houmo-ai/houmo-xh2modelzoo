...
# Description:
#   Qwen3 Embedding QuaRot rotation utilities adapted for the xh2 model zoo (xh2modelzoo).
...
from xh_model_zoo.xh_llm.quarot.hadamard_utils import is_pow2, random_hadamard_matrix
...
def bake_mean_into_linear_sensevoice(linear: torch.nn.Linear) -> None:
    """
    This function takes a linear layer and subtracts the means from the
    weights and biases. This will result in the linear layer performing
    the mean substitution which is usually done inside layernorm.
    For SenseVoice, we bake mean over dim=-1.
    """
    linear_dtype = linear.weight.dtype
    W_ = linear.weight.data.double()
...
def fuse_deepstack_linear(model):
    for layer in model.visual.deepstack_merger_list:
        ...
        model_utils.replace_modules(
            layer,
            torch.nn.LayerNorm,
            lambda x: model_utils.Qwen3RMSNorm(rms_weight_size, eps=x.eps),
            False,
        )
...
def fuse_layer_norms(model, device=None, llm_rotate=True):
    ...
    for layer in progress_bar:
        ...
        elif model_type == model_utils.SENSEVOICE_SMALL_MODEL:
            fuse_ln_linear(layer.norm2, [layer.feed_forward.w_1])
            fuse_ln_linear(
                layer.norm1,
                [
                    layer.self_attn.linear_q_k_v,
                ],
            )
            bake_mean_into_linear(layer.feed_forward.w_1)
            bake_mean_into_linear(layer.self_attn.linear_q_k_v)
            pass
        else:
    ...
    elif model_type == model_utils.QWEN3_BASE_MODEL:
        # Qwen3-Embedding has no lm_head; keep the final RMSNorm intact.
        pass
    else:
...
def rotate_qwen2_5_vl_ov_proj(layer, head_num, head_dim, is_visual=False):
    if is_visual:
        ...
        qkv.weight.data = torch.cat(
            [split_qkv_weight[0], split_qkv_weight[1], v_weight], dim=0
        )
        if qkv.bias is not None:
            ...
            qkv.bias.data = torch.cat(
                [split_qkv_bias[0], split_qkv_bias[1], v_bias], dim=0
            )
...
def rotate_qwen2_5_vl_visual_model(visual_model):
    ...
    Q_v = get_orthogonal_matrix(
        visual_model.blocks[0].attn.qkv.in_features, mode="hadamard"
    )
...
def rotate_visual_patch_pos_embed(model, Q_v: torch.Tensor):
    model.visual.pos_embed.weight.data = torch.matmul(
        model.visual.pos_embed.weight.data.double(), Q_v
    ).to(model.visual.pos_embed.weight.dtype)
...
def rotate_qwen3_vl_visual_merger(model, Q: torch.Tensor) -> None:
    ...
    W_ = model.visual.merger.linear_fc1.weight.to(dtype=torch.float64).reshape(
        o_shape, -1, q_shape
    )
...
def rotate_qwen3_vl_deepstack_merger(model, Q: torch.Tensor) -> None:
    for layer in model.visual.deepstack_merger_list:
        ...
        W_ = layer.linear_fc1.weight.to(dtype=torch.float64).reshape(
            o_shape, -1, q_shape
        )
        layer.linear_fc1.weight.data = (
            torch.matmul(W_, Q).to(dtype=dtype).reshape(o_shape, i_shape).contiguous()
        )
...
def rotate_embeddings(model, Q: torch.Tensor) -> None:
    ...
    for W in model_utils.get_embeddings(model, model_type):
        ...
        if W_.shape[-1] != Q.shape[0]:
            ...
            W.weight.data = (
                torch.matmul(W_, Q).to(dtype=dtype).to(raw_device).reshape(origin_shape)
            )
        else:
    ...
    if model_type == model_utils.QWEN3MOE_MODEL:
        for expert in layer.mlp.experts:
            ...
            if Q.shape[0] != W_.shape[0]:
                ...
                W_ = W_.reshape(Q.shape[0], -1)
                W.weight.data = (
                    torch.matmul(Q.T, W_).to(dtype=dtype).reshape(origin_shape)
                )
            else:
                W.weight.data = torch.matmul(Q.T, W_).to(dtype=dtype)
            ...
            if W.bias is not None:
                ...
                if Q.shape[0] != b.shape[0]:
                    ...
                    b = b.reshape(Q.shape[0], -1)
                    W.bias.data = (
                        torch.matmul(Q.T, b).to(dtype=dtype).reshape(origin_shape)
                    )
                else:
                    W.bias.data = torch.matmul(Q.T, b).to(dtype=dtype)
        return
...
def rotate_head(model, Q: torch.Tensor) -> None:
    # Rotate the head.
    model_type = model_utils.model_type_extractor(model)
    W = model_utils.get_lm_head(model, model_type=model_type)
    if W is None:
        return
    raw_device = W.weight.device
    W = W.to(Q.device)
...
def rotate_model(model, rotate_mode, device, quarot_matrix_size=None):
    ...
    if quarot_matrix_size is None:
        ...
        assert (
            int(llm_hidden_size / quarot_matrix_size) * quarot_matrix_size
            == llm_hidden_size
        ), "not fully dividen"
        for i in range(0, llm_hidden_size, quarot_matrix_size):
            local_Q = get_orthogonal_matrix(
                quarot_matrix_size, rotate_mode, device=device
            )
    ...
    if model_type == model_utils.QWEN3_VL_MODEL:
        ...
        elif hasattr(config, "text_config") and hasattr(config.text_config, "head_dim"):
    ...
    if model_type == model_utils.QWEN3_BASE_MODEL and rotate_mode == "hadamard":
        # Insert a post-norm linear to fold RMSNorm scale and cancel the final XQ rotation.
        hidden_size = model.config.hidden_size
        linear = torch.nn.Linear(hidden_size, hidden_size, bias=False)
        gamma = model.norm.weight.data.to(dtype=torch.float64, device=Q.device)
        # We want y = (x @ Q) @ (Q^T @ diag(gamma)), so set W = diag(gamma) @ Q
        W = gamma.unsqueeze(1) * Q
        linear.weight.data = W.to(dtype=model.norm.weight.data.dtype).contiguous()
        model.post_norm_linear = linear
        model.norm.weight.fill_(1.0)
        model.norm.fuse_weight = True
    rotate_head(model, Q)
    utils.cleanup_memory()
...
def add_qk_rotation_wrapper_after_function_call_in_forward(
    module, function_name, *args, **kwargs
):
    """
