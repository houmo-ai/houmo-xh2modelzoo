...
class _Qwen3RotaryEmbedding(DynamicModule):
    def _setup(self, cfg: Optional[Dict] = None):
        assert "dynamic" not in self.rope_type, (
            f"{self.rope_type} is not supported in dynamic mode"
        )
    ...
    def _setup_cos_sin_cache(self, seq_len, dtype):
        position_ids = torch.arange(
            0, seq_len, dtype=torch.long, device=self.inv_freq.device
        ).unsqueeze(0)
        ...
        cos = cos.to(self.inv_freq.device)
        sin = sin.to(self.inv_freq.device)
...
class _Qwen3Attention(DynamicModule):
    ...
    def apply_rotary_pos_emb(
        self, q: Tensor, k: Tensor, cos: Tensor, sin: Tensor, unsqueeze_dim: int = 1
    ):
        if self.enable_rope:
...
class _Qwen3Model(DynamicModule):
    def forward(
        ...
        if inputs_embeds is None:
            raise ValueError("inputs_embeds is required for qwen3-embedding export")

        if past_seq_length is None or current_input_length is None:
            raise ValueError("past_seq_length and current_input_length are required")
        ...
        if self.num_logits_to_keep == 0:
            # keep all token hidden states
            pass
        else:
            # EOS pooling: keep last valid token (inputs are expected to end with EOS)
            hidden_states = self.llm_gather(hidden_states, current_input_length - 1)
        hidden_states = self.norm(hidden_states)
        if hasattr(self, "post_norm_linear"):
    ...
    def _setup(self, cfg: Optional[Dict] = None):
        ...
        self.sin_slice._update_cfg = types.MethodType(
            _sin_cos_slice_update_cfg, self.sin_slice
        )
        self.cos_slice._update_cfg = types.MethodType(
            _sin_cos_slice_update_cfg, self.cos_slice
        )
