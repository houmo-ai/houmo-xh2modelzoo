...
# Description:
#   Qwen2 Hf Compatible implementation.
#
...
from transformers import AutoConfig, AutoModelForCausalLM, DynamicCache, GenerationMixin, Qwen2ForCausalLM
...
T = TypeVar("T", bound=Qwen2ForCausalLM)


def get_empty_hf_model(hf_model_dir, device_map="cpu", **kwargs) -> Qwen2ForCausalLM:
    """
    仅仅加载模型结构,不初始化权重,不占用显存
    """
    config = AutoConfig.from_pretrained(hf_model_dir)
    with no_init_weights(), init_empty_weights():
        hf_model: Qwen2ForCausalLM = AutoModelForCausalLM.from_config(
            config,
...
class Qwen2LegacyHFCompatible(Qwen2ForCausalLM):
    # _prefill: bool = True
    # _past_seq_length: int = 0
    # _embed_tokens: Optional[torch.nn.Module] = None
    # _llm_model: Optional[Qwen2LegacyInference] = None

    def setup(self, llm_model: Qwen2LegacyInference, embed_tokens: torch.nn.Module):
        self._prefill = False
    ...
    def forward(
        ...
        **kwargs,
        ...
        >>> model = Qwen2ForCausalLM.from_pretrained("Qwen/Qwen3-8B")
    ...
    def to_hf_compatible(
        ...
        llm_model: Optional[Qwen2LegacyInference] = None,
    ) -> Qwen2ForCausalLM:
        """
        ...
        if llm_model is not None:
            assert isinstance(llm_model, Qwen2LegacyInference)
            hf_model.__class__ = cls
            assert isinstance(hf_model, Qwen2LegacyHFCompatible)
    ...
    def generate(self, *args: Any, **kwargs: Any) -> torch.LongTensor:
        ...
        class ForwardContext:
            def __init__(self, model: Qwen2LegacyHFCompatible):
                self._model = model
