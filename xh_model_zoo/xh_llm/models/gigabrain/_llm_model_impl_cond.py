...
from xhquant.nn import LLMCache, AdaRMSNorm, Rope, RMSNorm
...
# ============================================================
# Gemma2 RMSNorm Wrapper (AdaRMS for Expert)
# ============================================================
...
class _Gemma2RMSNorm(DynamicModule):
    def forward(self, hidden_states, cond=None):
        if not hasattr(self, 'dense'):
            # 如果原始层没有 dense，直接当做普通 RMSNorm（兼容可能混用的情况）
            return self.norm_ori(hidden_states)

        modulation = self.dense(cond)
        # modulation shape: [batch, 3 * dim]
        modulation = modulation.unsqueeze(1)
        scale, shift, gate = torch.split(modulation, [self.dim, self.dim, self.dim], dim=-1)
        weight = 1.0 + scale#.to(torch.float32)
        output = self.norm(hidden_states, weight)
        output = output + shift#.to(torch.float32)
        return output, gate#.to(torch.float32)

    def _setup(self, cfg: Optional[Dict] = None):
        self.dim = self.weight.shape[0]
        if hasattr(self, 'dense'):
            self.norm = AdaRMSNorm(self.dim, self.eps)
            self.cond_dim = self.dense.weight.shape[1]
        else:
            self.norm_ori = RMSNorm(self.dim, self.eps)
            self.norm_ori.weight = nn.Parameter(deepcopy(self.weight.data + 1))
        return self


# ============================================================
# Gemma2 Attention Wrapper (Expert)
# ============================================================
...
# ============================================================
# Gemma2 Decoder Layer Wrapper (Expert)
# ============================================================
...
class _Gemma2DecoderLayer(DynamicModule):
    def forward(
        ...
        **kwargs,
        ...
        # Pre-attention norm (AdaRMS)
        hidden_states, gate1 = self.input_layernorm(hidden_states, cond)
        ...
        # Post-attention norm (Standard RMS)
        hidden_states = self.post_attention_layernorm(hidden_states)

        # Gated Residual connection
        hidden_states = residual + gate1 * hidden_states

        # MLP block
        residual = hidden_states

        # Pre-feedforward norm (AdaRMS)
        hidden_states, gate2 = self.pre_feedforward_layernorm(hidden_states, cond)

        hidden_states = self.mlp(hidden_states)

        # Post-feedforward norm (Standard RMS)
        hidden_states = self.post_feedforward_layernorm(hidden_states)

        # Gated Residual connection
        hidden_states = residual + gate2 * hidden_states
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            layer_outputs = decoder_layer(
                hidden_states,
                cond=cond,  # 传递 cond 参数给带有 AdaRMS 的 decoder layer
                past_seq_length=past_seq_length,
                current_input_length=current_input_length,
        ...
        # Final norm 也可能是 AdaRMSNorm
        if hasattr(self.norm, 'dense'):
            hidden_states, _ = self.norm(hidden_states, cond)
        else:
...
# ============================================================
# Gemma2 For Causal LM Wrapper (Expert)
# ============================================================
...
class _Gemma2ForCausalLM(DynamicModule):
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            cond=cond,  # 传递 cond 参数给 Expert Model
            past_seq_length=past_seq_length,
            current_input_length=current_input_length,
            attention_mask=attention_mask,
