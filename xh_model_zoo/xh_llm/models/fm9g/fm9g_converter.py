...
#
# File: fm9g_converter.py
# Description:
#   FM9G Converter implementation.
#   FM9G is a custom model loaded via trust_remote_code=True.
#
...
class FM9GConverterXH2a(HFTransfromersConverter):
    target_device = DeviceType.XH2a

    def __init__(self, config: FM9GConvertConfig):
        super().__init__()
        self.config = config
    ...
    def _convert(self, hf_model_path: str, output_dir: str):
        ...
        # FM9G 是 trust_remote_code 模型,需要复制模型代码文件
        for py_file in Path(hf_model_path).glob("*.py"):
            dst_file = Path(hf_config_dir) / py_file.name
            shutil.copyfile(py_file, dst_file)
        ...
        from ._model_impl import (
            register_wrap_cls as fm9g_register_wrap_cls,
        )

        fm9g_register_wrap_cls(native_model)
    ...
    @classmethod
    def convert(cls, hf_model_path: str, config: FM9GConvertConfig, output_dir: str):
        # quant_config = create_quant_config(config.quant_scheme)
        # is_ssfp = is_ssfp_quant_config(quant_config)
        # if is_ssfp:
        #     assert config.quant_weight is not None and Path(config.quant_weight).exists()
        FM9GConverterXH2a(config)._convert(hf_model_path, output_dir)
