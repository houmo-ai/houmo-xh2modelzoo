...
from ._llm_model_impl import _GlmAsrLanguageModel
from transformers.modeling_outputs import BaseModelOutputWithPast

from ..base_llm_model import LLMBaseModel
from ..builder import MODELS

from xh_model_zoo.xh_llm.models.glm_asr.modeling_glmasr import (
    GlmAsrForConditionalGeneration
)


@MODELS.register_module()
class XHGlmAsrLLMModel(LLMBaseModel):
    def __init__(
        self,
        hf_model: str,
        wrap_cfg,
    ...
    def init_wrap_model(self, hf_model: Optional[torch.nn.Module] = None):
        from ._llm_model_impl import register_wrap_cls as llm_register_wrap_cls

        # For GLM-ASR, hf_model is LlamaForCausalLM, we need its .model (LlamaModel)
        # which contains embed_tokens and the decoder layers
        if hasattr(hf_model, 'model'):
            llm_model = hf_model.model  # LlamaModel
        else:
            llm_model = hf_model

        llm_register_wrap_cls(llm_model)
        super().init_wrap_model(llm_model)

        _llm = self.wrap_model
        self.config = _llm.config
        self.token_embedding = _llm.embed_tokens
        # generation_config may not exist on LlamaModel, try to get from parent
