...
class _Qwen3OmniMoeVisionPatchEmbed(DynamicModule):
    ...
    def _setup(self, cfg: ConfigDict):
        ...
        # Pre-compute constants to avoid FloorDiv in FX graph (unsupported by quantizer)
        max_size_t = cfg.get("max_size_t", 2) if isinstance(cfg, dict) else getattr(cfg, "max_size_t", 2)
        self._t_new = max_size_t // self.temporal_patch_size
        h_out = self.max_size_h // self.patch_size
        w_out = self.max_size_w // self.patch_size
        self._h_half = h_out // 2
        self._w_half = w_out // 2

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # hidden_states: [b, c, t, h, w]
        b, c, t, h, w = hidden_states.shape
        hidden_states = hidden_states.reshape(b, c, self._t_new, self.temporal_patch_size, h, w)
        hidden_states = hidden_states.permute(0, 2, 1, 3, 4, 5).reshape(-1, c, self.temporal_patch_size, h, w)
        ...
        hidden_states = hidden_states.reshape(b, self._t_new, c, self._h_half, 2, self._w_half, 2)
