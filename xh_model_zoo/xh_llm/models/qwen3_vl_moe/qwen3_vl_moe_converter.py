...
#
# File: qwen3_vl_moe_converter.py
# Description:
#   Qwen3 Vl Moe Converter implementation.
#
...
class Qwen3_VL_MOEConverterXH2a(HFTransfromersConverter):
    ...
    def load_hf_model(self, hf_model_path, **kwargs) -> Qwen3VLMoeForConditionalGeneration:
        native_model = Qwen3VLMoeForConditionalGeneration.from_pretrained(hf_model_path, **kwargs)
    ...
    def _export_vision(
        self, inputs, hf_model: Qwen3VLMoeForConditionalGeneration, vision_hmonnx_file: str, golden_dir: str
    ):
    ...
    def _convert(self, hf_model_path: str, output_dir: str):
        ...
        if resume_from is not None:
            if config.convert_moe_linear:
                from xh_model_zoo.xh_llm.quarot.gptq_utils import Qwen3VLMoeTextExperts_Linearized
                num_layers = len(native_model.language_model.layers)
                for i in range(num_layers):
                    native_model.language_model.layers[i].mlp.experts=Qwen3VLMoeTextExperts_Linearized(native_model.language_model.layers[i].mlp.experts)
    ...
    def convert(cls, hf_model_path: str, config: Qwen3_VLConvertConfig, output_dir: str):
        ...
        Qwen3_VL_MOEConverterXH2a(config)._convert(hf_model_path, output_dir)
