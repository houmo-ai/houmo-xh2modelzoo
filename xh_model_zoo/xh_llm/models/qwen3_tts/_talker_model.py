...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        Qwen3TTSTalkerRotaryEmbedding: "Qwen3TTSTalkerRotaryEmbedding",
    }
)
class _Qwen3TTSTalkerRotaryEmbedding(DynamicModule):
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        Qwen3TTSTalkerAttention: "Qwen3TTSTalkerAttention",
    }
)
class _Qwen3TTSTalkerAttention(DynamicModule):
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        Qwen3TTSTalkerDecoderLayer: "Qwen3TTSTalkerDecoderLayer",
    }
)
class _Qwen3TTSTalkerDecoderLayer(DynamicModule):
    def forward(
        self,
        hidden_states: torch.Tensor,
        past_seq_length: Tensor | None = None,
...
class _Qwen3TTSTalkerModel(DynamicModule):
    def _setup_cos_sin_embeding(self):
        ...
        def apply_multimodal_cos_sin_emb(cos, sin, mrope_section, mrope_interleaved=False, unsqueeze_dim=1):
        ...
        cos_cached, sin_cached = apply_multimodal_cos_sin_emb(
            cos_cached, sin_cached, self.config.rope_scaling["mrope_section"], self.config.rope_scaling["interleaved"]
        )
        self.rotary_emb.cos_cached = cos_cached
        self.rotary_emb.sin_cached = sin_cached
    ...
    def forward(  # type: ignore[override]
        ...
        causal_mask = None  # 在Attention中会动态生成
...
def register_wrap_modules(talker: Qwen3TTSTalkerForConditionalGeneration | None = None):
    pass
