...
#
# File: modeling_paddleocr_vl.py
# Description:
#   Modeling implementation for paddleocr_vl_1_5 in HOUMO AI xh2modelzoo.
#
...
class Projector(nn.Module):
    ...
    def __init__(
        ...
        # 强制 float32：PyTorch 2.8 的 LayerNorm 要求输入和参数 dtype 完全一致，
        # 模型以 float16 加载后 LayerNorm 权重也是 float16，forward 中 upcast 输入
        # 到 float32 后会与 float16 权重冲突，因此这里直接把 pre_norm 固定为 float32。
    ...
    def forward(
        ...
        if isinstance(image_features, (list, tuple)):
            ...
            for image_feature, image_grid in zip(image_features, image_grid_thw):
                orig_dtype = image_feature.dtype
                # PyTorch 2.8 LayerNorm 要求输入和权重 dtype 一致，显式 upcast 到 float32
                image_feature = F.layer_norm(
                    image_feature.float(),
        ...
        hidden_states = (
            F.layer_norm(
                ...
                self.pre_norm.bias.float() if self.pre_norm.bias is not None else None,
                self.pre_norm.eps,
            )
            .to(orig_dtype)
