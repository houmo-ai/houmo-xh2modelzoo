...
def _cat_exported_single_batch_inputs(value, batch_size: int, dim: int = 0):
    """Concat graph inputs that are intentionally provided as per-batch lists.

    TorchFX represents list arguments as Proxy placeholders, so an
    ``isinstance(value, list)`` guard is false while tracing.  For exported
    continue-batch models we therefore index the fixed-length list
    unconditionally to create integer ``getitem`` nodes that xhquant can
    expand into separate ONNX inputs.
    """
...
def _normalize_linear_conv_cache_rank(conv_cache):
    if isinstance(conv_cache, (list, tuple)):
        return tuple(_normalize_linear_conv_cache_rank(c) for c in conv_cache)
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {Qwen3_5MoeTextRotaryEmbedding: "Qwen3_5MoeTextRotaryEmbedding"}
)
class _Qwen3_5MoeTextRotaryEmbedding(DynamicModule):
    ...
    def _compute_cos_sin(self, max_seq_len=2048):
        ...
        inv_freq_expanded = (
            ...
            .float()
            .expand(1, max_seq_len, -1, 1)
        )
        positions = torch.arange(
            max_seq_len, device=self.inv_freq.device
        ).float()[None, :, None, None]

        device_type = (
            self.inv_freq.device.type
            if isinstance(self.inv_freq.device.type, str)
            and self.inv_freq.device.type != "mps"
            else "cpu"
        )
...
class _Qwen3_5MoeGatedDeltaNet(DynamicModule):
    ...
    def forward(
        ...
        if self.split_conv_cache:
            # --- Split conv_cache path: separate q/k/v convolutions ---
            mixed_qkv_t = mixed_qkv.transpose(1, 2)  # [bs, conv_dim, seq]
            ...
            value_out = F.silu(value_out).to(mixed_qkv_t.dtype)
            mask_qkv = linear_attn_mask.unsqueeze(1)
            query_out = query_out * mask_qkv
            key_out = key_out * mask_qkv
    ...
    def _update_cfg(self, cfg: Optional[Dict] = None):
        ...
        self.split_conv_cache = cfg.get(
            "split_conv_cache",
            getattr(self, "split_conv_cache", False),
        )
...
class _Qwen3_5MoeTextModel(DynamicModule):
    ...
    def _setup(self, cfg):
        ...
        for idx_layer, decoder_layer in enumerate(self.layers):
            if (
                self.layer_types[idx_layer] == "linear_attention"
                and idx_layer in alpha_scaling_layers
            ):
    ...
    def forward(
        ...
        for idx_layer, decoder_layer in enumerate(self.layers):
            ...
            if self.use_cache:
                if layer_type == "full_attention":
                    if self.batch_size > 1:
                        ...
                        _past_k_cache = (
                            past_key_cache[full_attn_cache_idx] if past_key_cache is not None else None
                        )
                        _past_v_cache = (
            ...
            if layer_type == "linear_attention":
                ...
                # Collect caches from linear_attention layers.  The exported
                # continue-batch graph contract requires every output with a
                # batch dimension to be split into one single-batch output per
                ...
                def _extend_single_batch(out_list, value):
                    if isinstance(value, (list, tuple)):
                        ...
                        # FX sees module outputs as Proxy objects, not Tensor,
                        # so split the fixed exported batch unconditionally.
                        out_list.extend(
...
class _Qwen3_5MoeForCausalLM(DynamicModule):
    ...
    def forward(
        ...
        if self._has_extra_hidden_output:
            ...
            extra_hidden = _split_single_batch_outputs(extra_hidden, self.batch_size)
