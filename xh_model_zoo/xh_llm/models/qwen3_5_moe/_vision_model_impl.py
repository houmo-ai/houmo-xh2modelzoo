...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen3_5MoeVisionAttention: "Qwen3_5MoeVisionAttention"})
class _Qwen3_5MoeVisionAttention(DynamicModule):
    ...
    def _setup(self, cfg: ConfigDict):
        ...
        if head_dim % 64 != 0:
            ...
            proj_out_features, _ = self.proj.weight.shape
...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen3_5MoeVisionBlock: "Qwen3_5MoeVisionBlock"})
class _Qwen3_5MoeVisionBlock(DynamicModule):
    @torch.no_grad()
    def _setup(self, cfg: ConfigDict):
        pass
...
def weight_denorm(conv2d: nn.Conv2d):
...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen3_5MoeVisionPatchEmbed: "Qwen3_5MoeVisionPatchEmbed"})
class _Qwen3_5MoeVisionPatchEmbed(DynamicModule):
    @torch.no_grad()
    def _setup(self, cfg: ConfigDict):
        self.only_first_block = False
        self.max_size_w = cfg.max_size_w
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = hidden_states.reshape(b, t_new, channels, height // 2, 2, width // 2, 2)
        hidden_states = hidden_states.permute(0, 1, 3, 5, 4, 6, 2)
        hidden_states = hidden_states.reshape(b, -1, channels)
        return hidden_states


@XHLLM_TRACEABLE_MODULES.register_module({Qwen3_5MoeVisionPatchMerger: "Qwen3_5MoeVisionPatchMerger"})
class _Qwen3_5MoeVisionPatchMerger(DynamicModule):
    def _setup(self, cfg: ConfigDict):
        pass
...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen3_5MoeVisionModel: "Qwen3_5MoeVisionModel"})
class _Qwen3_5MoeVisionModel(DynamicModule):
    @torch.no_grad()
    def _setup(self, cfg: ConfigDict):
        self.only_first_block = False
        self.patch_size = cfg.patch_size
