...
# Description:
#   Qwen3 Hf Compatible implementation.
#
...
class Qwen3LegacyLoRAHFCompatible(Qwen3ForCausalLM):
    # _prefill: bool = True
    # _past_seq_length: int = 0
    # _embed_tokens: Optional[torch.nn.Module] = None
    # _llm_model: Optional[Qwen3LegacyInference] = None

    def setup(self, llm_model: Qwen3LegacyLoRAInference, embed_tokens: torch.nn.Module):
        self._prefill = False
    ...
    def to_hf_compatible(
        ...
        llm_model: Optional[Qwen3LegacyLoRAInference] = None,
    ) -> Qwen3ForCausalLM:
        """
        ...
        if llm_model is not None:
            assert isinstance(llm_model, Qwen3LegacyLoRAInference)
            hf_model.__class__ = cls
            assert isinstance(hf_model, Qwen3LegacyLoRAHFCompatible)
    ...
    def generate(self, *args: Any, **kwargs: Any) -> torch.LongTensor:
        ...
        class ForwardContext:
            def __init__(self, model: Qwen3LegacyLoRAHFCompatible):
                self._model = model
