...
# File: quant_utils.py
# Description:
#   Qwen3 legacy LoRA QuaRot quantization helpers adapted for the xh2 model zoo (xh2modelzoo).
    ...
    def forward(self, x):
        ...
        if self.online_full_had:
            ...
            if self.fp32_had:  # Full Hadamard in FP32
                if x.shape[1] == 1408:
                    x = torch.nn.functional.pad(x, (0, 256))
                elif x.shape[1] == 5632:
                    x = torch.nn.functional.pad(x, (0, 1024))
                if x.shape[0] != 0:
...
class WeightQuantizer(torch.nn.Module):
    ...
    def find_params(self, x, H=None):
        ...
        if self.perchannel:
            x = x.flatten(1)
        # elif self.perchannel and groupsize!=-1:
        #     x_row = shape[0]
        #     x = x.reshape(-1,64).flatten(1)
        else:
