...
class Qwen3HFCompatible(Qwen3ForCausalLM):
    # _prefill: bool = True
    # _past_seq_length: int = 0
    # _embed_tokens: Optional[torch.nn.Module] = None
    # _llm_model: Optional[Qwen3Inference] = None

    def setup(self, llm_model: Qwen3Inference, embed_tokens: torch.nn.Module):
        self._prefill = False
    ...
    def to_hf_compatible(
        ...
        llm_model: Optional[Qwen3Inference] = None,
    ) -> Qwen3ForCausalLM:
        """
        ...
        if llm_model is not None:
            assert isinstance(llm_model, Qwen3Inference)
            hf_model.__class__ = cls
            assert isinstance(hf_model, Qwen3HFCompatible)
    ...
    def generate(self, *args: Any, **kwargs: Any) -> torch.LongTensor:
        ...
        class ForwardContext:
            def __init__(self, model: Qwen3HFCompatible):
                self._model = model
