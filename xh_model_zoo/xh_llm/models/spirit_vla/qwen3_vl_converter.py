...
#
# File: qwen3_vl_converter.py
# Description:
#   Qwen3 VL model converter implementation.
#   This module provides Qwen3_VLConverterXH2a class for converting
#   Qwen3 VL models to HMONNX format.
#
...
from .modeling_qwen3_vl import Qwen3VLForConditionalGeneration, Qwen3VLVisionModel
...
class Qwen3_VLConverterXH2a(HFTransfromersConverter):
    """Qwen3_VL Converter for XH2a."""
    ...
    def _convert(self, hf_model_path: str, output_dir: str):
        ...
        if config.gptqmodel_cfg:
            ...
            ori_model = self.load_hf_model(
                hf_model_path, trust_remote_code=True, torch_dtype=torch.float16, device_map="cpu"
            )
            # from xh_model_zoo.xh_llm.quarot import rotation_utils
            # rotation_utils.fuse_layer_norms(native_model)
        native_model.load_state_dict(ori_model.state_dict(), strict=False)
        del ori_model
        native_model.eval()    
        # 融合GPTQ权重
        ...
        for candidates in hf_config_file_candidates:
            ...
            for cfg_file in candidates:
                src_file = Path(pretrin_model_path) / cfg_file
                if src_file.exists():
        ...
        processor = Qwen3VLProcessor.from_pretrained(pretrin_model_path)
        ...
        data_prefill = {
            ...
            "deepstack_image_embeds": [ds_vision.to(torch.float32) for ds_vision in deepstack_image_embeds],
            "past_seq_length": 0,
    ...
    def convert(cls, hf_model_path: str, config: Qwen3_VLConvertConfig, output_dir: str):
        ...
        Qwen3_VLConverterXH2a(config)._convert(hf_model_path, output_dir)
