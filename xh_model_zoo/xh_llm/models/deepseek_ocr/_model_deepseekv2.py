...
#
# File: _model_deepseekv2.py
# Description:
#   Model Deepseekv2 model implementation.
#
...
import math
import types
from typing import Optional, Tuple, List, Dict, Type, Union, Any
...
from xh_model_zoo.xh_llm.models.deepseek_ocr.modeling_deepseekv2 import (
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        DeepseekV2ForCausalLM: "deepseek_ocr.llm.DeepseekV2ForCausalLM",
    }
)
class _DeepseekV2ForCausalLM(DynamicRegister):
    def _setup(self, cfg):
        return self
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        DeepseekV2Model: "deepseek_ocr.llm.DeepseekV2Model",
    }
)
class _DeepseekV2Model(DynamicRegister):
    def forward(
        self,
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {DeepseekV2RMSNorm: "deepseek_ocr.llm.DeepseekV2RMSNorm",}
    )
class _DeepseekV2RMSNorm(DynamicRegister):
...
@XHLLM_TRACEABLE_MODULES.register_module(
    { LlamaRotaryEmbedding: "deepseek_ocr.llm.LlamaRotaryEmbedding",}
)
class _RotaryEmbedding(DynamicRegister):
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {LlamaAttention: "deepseek_ocr.llm.LlamaAttention",}
)
class _LlamaAttention(DynamicRegister):
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        DeepseekV2DecoderLayer: "deepseek_ocr.llm.DeepseekV2DecoderLayer",
    }
)
class _DeepseekV2DecoderLayer(DynamicRegister):
    def forward(
        self,
        hidden_states: torch.Tensor,
        # position_ids: Optional[torch.LongTensor] = None,
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        MoEGate: "deepseek_ocr.llm.MoEGate",
    }
)
class _MoEGate(DynamicRegister):
    def _setup(self, cfg: Optional[Dict] = None):
        self.proj = torch.nn.Linear(self.weight.shape[-1], self.weight.shape[0], bias=None)
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        DeepseekV2MoE: "deepseek_ocr.llm.DeepseekV2MoE",
    }
)
class _DeepseekV2MoE(DynamicRegister):
    def forward(self, hidden_states):
        ...
        # routing_weights = torch.zeros((bsz, seq_len, self.num_experts_per_tok), device=self.device, dtype=hidden_states.dtype)
        # routing_weights = routing_weights.view(bsz * seq_len, self.num_experts_per_tok)
        # routing_weights[topk_idx] = topk_weight
        y = self.moeblock(hidden_states,   routing_weights)
        if self.config.n_shared_experts is not None:
            y = y + self.shared_experts(ori_hidden_states)
        return y
    ...
    def _setup(self, cfg: Optional[Dict] = None):
        ...
        self.moeblock = xhnn.MoeBlock(self.experts[0].act_fn._get_name().lower(), self.gate.top_k, self.gate.norm_topk_prob)
