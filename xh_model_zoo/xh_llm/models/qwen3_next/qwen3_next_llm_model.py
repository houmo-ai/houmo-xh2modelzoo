...
#
# File: qwen3_next_llm_model.py
# Description:
#   Qwen3Next LLM model wrapper with mixed full/linear attention cache support.
#
...
class XHQwen3NextModel(LLMBaseModel):
    ...
    def get_hf_model(self, device_map="cpu", **kwargs) -> Qwen3NextForCausalLM:
        hf_model = super().get_hf_model(device_map, **kwargs)
        qwen3next_hf_model = cast(Qwen3NextForCausalLM, hf_model)
        assert id(qwen3next_hf_model.get_output_embeddings().weight) != id(
            qwen3next_hf_model.get_input_embeddings().weight
        )
        return qwen3next_hf_model
    ...
    def _load_hf_model(self, device_map="cpu", **kwargs):
        ...
        if device_map == "meta":
            with no_init_weights():
                hf_model = Qwen3NextForCausalLM(config)
        else:
    ...
    def _load_gptqmodel(self, device_map="cpu", **kwargs):
        ...
        # Manually apply defuser patch to replace fused MoE blocks with defused
        # (linear) ones. This is needed because defuser.replace_fused_blocks()
        ...
        hf_model = cast(Qwen3NextForCausalLM, q_model.model.eval())
    ...
    def init_wrap_model(self, hf_model=None):
        ...
        self.head_dim = hf_model.config.head_dim
