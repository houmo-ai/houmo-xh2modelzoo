...
from .qwen3_convert_config import Qwen3LegacyConverterXH2a_LLM_config, Qwen3LegacyConverterXH2a_Expert_config
...
class Qwen3LegacyConverterXH2a_LLM(HFTransfromersConverter):
    target_device = DeviceType.XH2a

    def __init__(self, config: Qwen3LegacyConverterXH2a_LLM_config):
        super().__init__()
        self.config = config
    ...
    def load_hf_model(self, hf_model_dir: str, **kwargs):
        native_model = DM0ForCausalLM.from_pretrained(
            hf_model_dir,
            torch_dtype=torch.float32,
            low_cpu_mem_usage=True,
            trust_remote_code=True,
        ).to("cpu")
        native_model.model.llm.lm_head = native_model.lm_head
        return native_model.model.llm
    ...
    def _convert(self, hf_model_path: str, output_dir: str):
        ...
        attention_mask = torch.randn((1, 1, input_sequence_length, context_length), dtype=torch.float16)
            ...
            with torch.no_grad():
                ...
                for arg in inputs:
                    if isinstance(arg, (list, tuple)):
                        ...
                        if arg.dtype == torch.float32 or arg.dtype == torch.bfloat16:
                            arg = arg.to(torch.float16)
    ...
    @classmethod
    def convert(cls, hf_model_path: str, config: Qwen3LegacyConverterXH2a_LLM_config, output_dir: str):
        quant_config = create_quant_config(config.quant_scheme)
        ...
        Qwen3LegacyConverterXH2a_LLM(config)._convert(hf_model_path, output_dir)


class Qwen3LegacyConverterXH2a_Expert(HFTransfromersConverter):
    target_device = DeviceType.XH2a

    def __init__(self, config: Qwen3LegacyConverterXH2a_Expert_config):
        super().__init__()
        self.config = config
        self.hf_model_path: Optional[str] = None
        self.output_dir: Optional[str] = None
    ...
    def load_hf_model(self, hf_model_dir: str, **kwargs):
        ...
        return native_model.model.action_expert
    ...
    def _convert(self, hf_model_path: str, output_dir: str):
        ...
        inputs_embeds = torch.randn(1, input_sequence_length, wraped_qwen_model.model.config.hidden_size)
        past_seq_length_t = torch.tensor([0], dtype=torch.int32)
        current_input_length_t = torch.tensor(current_input_length, dtype=torch.int32)
        attention_mask = torch.randn((1, 1, input_sequence_length, context_length), dtype=torch.float16)
        ...
        prefill_golden_dir = str(work_dir / "golden" / f"{prefix}-expert-prefill")
        if not Path(prefill_golden_dir).exists():
            logger.info(f"start export vision model golden............")
            from xhquant.api import HMONNXGoldenInference
        ...
        # 更新与input_sequence_length相关的Module
        wrap_cfg.input_sequence_length = 50
        quanted_model.update_cfg(wrap_cfg)
        ...
        decoder_golden_dir = str(work_dir / "golden" / f"{prefix}-expert-decode")
        if not Path(decoder_golden_dir).exists():
            logger.info(f"start export decode model golden............")
            from xhquant.api import HMONNXGoldenInference
    ...
    @classmethod
    def convert(cls, hf_model_path: str, config: Qwen3LegacyConverterXH2a_Expert_config, output_dir: str):
        quant_config = create_quant_config(config.quant_scheme)
        is_ssfp = is_ssfp_quant_config(quant_config)
        ...
        if is_ssfp:
            ...
            if not hasattr(hf_config, "quantization_config"):
                assert config.quant_weight is not None and Path(config.quant_weight).exists()
        Qwen3LegacyConverterXH2a_Expert(config)._convert(hf_model_path, output_dir)