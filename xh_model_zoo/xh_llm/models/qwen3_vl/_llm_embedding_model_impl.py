...
#
# File: _llm_embedding_model_impl.py
# Description:
#   Embedding-specific LLM model implementation for Qwen3-VL-Embedding.
#   This module provides a wrapper that outputs last_hidden_states instead
#   of logits, removing the lm_head layer for embedding use cases.
#
...
# Copy all the wrapper classes from _llm_model_impl.py but register with "Embedding" suffix
# This ensures the entire wrapper chain uses embedding-specific logic


@XHLLM_TRACEABLE_MODULES.register_module({Qwen3VLTextRMSNorm: "Qwen3VLTextRMSNormEmbedding"})
class _Qwen3VLTextRMSNormEmbedding(DynamicModule):
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        Qwen3VLTextRotaryEmbedding: "Qwen3VLTextRotaryEmbeddingEmbedding",
    }
)
class _Qwen3VLTextRotaryEmbeddingEmbedding(DynamicModule):
...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen3VLTextAttention: "Qwen3VLTextAttentionEmbedding"})
class _Qwen3VLTextAttentionEmbedding(DynamicModule):
    def _setup(self, cfg: ConfigDict):
...
@XHLLM_TRACEABLE_MODULES.register_module({Qwen3VLTextDecoderLayer: "Qwen3VLTextDecoderLayerEmbedding"})
class _Qwen3VLTextDecoderLayerEmbedding(DynamicModule):
    def _setup(self, cfg: ConfigDict):
        pass
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        Qwen3VLTextModel: "Qwen3VLTextModelEmbedding",
    }
)
class _Qwen3VLTextModelEmbedding(DynamicModule):
    def _setup(self, cfg: ConfigDict):
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        Qwen3VLModel: "Qwen3VLModelEmbedding",
    }
)
class _Qwen3VLModelEmbedding(DynamicModule):
    def _setup(self, cfg: ConfigDict):
        self.cfg = cfg
        del self.visual
...
@XHLLM_TRACEABLE_MODULES.register_module(
    {
        Qwen3VLForConditionalGeneration: "Qwen3VLForConditionalGenerationEmbedding",
    }
)
class _Qwen3VLForConditionalGenerationEmbedding(DynamicModule):
    """
    Embedding-specific wrapper for Qwen3-VL-Embedding models.

    Unlike the generation wrapper (_Qwen3VLForConditionalGeneration), this wrapper:
    - Does NOT apply lm_head
    - Returns last_hidden_states directly
    - Suitable for embedding/retrieval tasks
    """
    ...
    def forward(
        ...
        """
        Forward pass that returns last_hidden_states without lm_head projection.

        Returns:
            Tensor: last_hidden_states with shape [batch, seq_len, hidden_dim]
                    or [batch, 1, hidden_dim] if num_logits_to_keep=1
        """
        ...
        # Return last_hidden_states directly, no lm_head
        hidden_states = outputs[0]
        return hidden_states


def register_wrap_cls(hf_model):
    """Register embedding wrapper for the model."""
    pass
