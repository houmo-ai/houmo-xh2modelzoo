...
#
# File: gpt_oss_hf_compatible.py
# Description:
#   Gpt Oss Hf Compatible implementation.
#
...
T = TypeVar("T", bound=GptOssForCausalLM)


def get_empty_hf_model(hf_model_dir, device_map="cpu", **kwargs) -> GptOssForCausalLM:
    """
    仅仅加载模型结构,不初始化权重,不占用显存
    """
    config = AutoConfig.from_pretrained(hf_model_dir)
    with no_init_weights(), init_empty_weights():
        hf_model: GptOssForCausalLM = AutoModelForCausalLM.from_config(
            config,
...
class GptOssWithMaskHFCompatible(GptOssForCausalLM):
    ...
    def forward(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        **kwargs,
        ...
        >>> model = GptOssForCausalLM.from_pretrained("gpt-oss-20b")
        ...
        assert inputs_embeds is not None, "inputs_embeds should not be None at this point"
        ...
        (
            ...
            past_key_caches,
            past_value_caches,
        ) = self._llm_model.prepare_inputs(input_data, pad_input_seq_length)  # pyright: ignore[reportAssignmentType]

        pad_seq_lenght = inputs_embeds.shape[1]  # pyright: ignore[reportOptionalMemberAccess]
        ...
        for i in range(steps):
            ...
            sub_inputs_embeds = inputs_embeds[:, start:end, :]  # pyright: ignore[reportOptionalSubscript]
            ...
            # Prepare attention masks for this step
            sub_local_attention_mask = None
            sub_global_attention_mask = None
            if local_attention_mask is not None:
                sub_local_attention_mask = local_attention_mask[:, start:end, :]
            if global_attention_mask is not None:
                sub_global_attention_mask = global_attention_mask[:, start:end, :]

            outputs = self._llm_model(
                sub_inputs_embeds,
                sub_past_seq_length,
                sub_current_input_length,
                sub_local_attention_mask,
                sub_global_attention_mask,
                past_key_caches,
                past_value_caches,
            )
        ...
        return CausalLMOutputWithPast(  # type: ignore
            # loss=loss,
    ...
    def to_hf_compatible(
        cls,
        hf_model_or_path: Union[GptOssForCausalLM, str],
        llm_model: Optional[GptOssWithMaskInference] = None,
    ) -> GptOssForCausalLM:
        """
        ...
        elif isinstance(hf_model_or_path, GptOssForCausalLM):
            hf_model = hf_model_or_path

        if llm_model is not None:
            assert isinstance(llm_model, GptOssWithMaskInference)
            hf_model.__class__ = cls
            assert isinstance(hf_model, GptOssWithMaskHFCompatible)
    ...
    def _sample_forward(
        ...
        **kwargs,
        ...
        out = self._original_forward(  # pyright: ignore[reportCallIssue]
            input_ids=input_ids,
    ...
    def generate(self, *args: Any, **kwargs: Any) -> torch.LongTensor:
        ...
        class ForwardContext:
            def __init__(self, model: GptOssWithMaskHFCompatible):
                self._model = model
                self._model._original_forward = self._model.forward  # pyright: ignore[reportArgumentType]
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                self._model.forward = self._model._original_forward  # pyright: ignore[reportAttributeAccessIssue]
...
# Backward compatible alias for old name used in __init__.py and scripts
GptOssWithMask_HFCompatible = GptOssWithMaskHFCompatible
