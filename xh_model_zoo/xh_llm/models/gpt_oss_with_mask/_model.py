...
class _GptOssMLP(DynamicModule):
    def forward(self, hidden_states):
        router_scores = self.router(hidden_states)  # (num_experts, seq_len)
...
class _GptOssExperts(DynamicModule):
    def forward(self, hidden_states: torch.Tensor, routing_weights=None) -> torch.Tensor:
        """
...
class _GptOssAttention(DynamicModule):
    ...
    def forward(
        ...
        if self.sliding_window is not None:
            causal_mask = local_attention_mask
        else:
            causal_mask = global_attention_mask
        ...
        attn_weights: Optional[Tensor] = self.softmax(attn_weights, sinks=sinks)
    ...
    def _setup(self, cfg: Union[ConfigDict, Dict[str, Any]]):
        ...
        self.softmax = SinksSoftmax(dim=-1)
...
def register_wrap_modules(hf_model: Optional[GptOssForCausalLM] = None):
    pass
