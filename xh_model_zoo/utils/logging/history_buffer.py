...
#
# File: history_buffer.py
# Description:
#   History buffer for logging statistics.
#   This module provides HistoryBuffer class for storing and
#   computing statistics from log history.
#
...
class HistoryBuffer:
    ...
    def update(self, log_val: Union[int, float], count: int = 1) -> None:
        ...
        if not isinstance(log_val, (int, float)) or not isinstance(count, (int, float)):
            raise TypeError(
                f"log_val must be int or float but got " f"{type(log_val)}, count must be int but got " f"{type(count)}"
            )
    ...
    def register_statistics(cls, method: Callable) -> Callable:
        ...
        assert method_name not in cls._statistics_methods, "method_name cannot be registered twice!"
    ...
    def statistics(self, method_name: str, *arg, **kwargs) -> Any:
        ...
        if method_name not in self._statistics_methods:
            raise KeyError(f"{method_name} has not been registered in " "HistoryBuffer._statistics_methods")
