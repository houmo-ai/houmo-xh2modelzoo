...
#   This module provides MessageHub class for managing runtime
#   information and log history during training.
#
...
class MessageHub(ManagerMixin):
    ...
    def __init__(
        ...
        for value in self._log_scalars.values():
            assert isinstance(value, HistoryBuffer), (
                "The type of log_scalars'value must be HistoryBuffer, but " f"got {type(value)}"
            )

        for key in self._resumed_keys.keys():
            assert key in self._log_scalars or key in self._runtime_info, (
                "Key in `resumed_keys` must contained in `log_scalars` or " f"`runtime_info`, but got {key}"
            )
    ...
    def update_scalar(
        ...
        assert isinstance(count, int), f"The type of count must be int. but got {type(count): {count}}"
    ...
    def update_scalars(self, log_dict: dict, resumed: bool = True) -> None:
        ...
        Examples:
            >>> message_hub = MessageHub.get_instance('xhquant')
            ...
            >>> # The count of `c` is 2.
        """
        assert isinstance(log_dict, dict), "`log_dict` must be a dict!, " f"but got {type(log_dict)}"
        for log_name, log_val in log_dict.items():
            if isinstance(log_val, dict):
                ...
                count = 1
                value = log_val
            assert isinstance(count, int), "The type of count must be int. but got " f"{type(count): {count}}"
    ...
    def update_info_dict(self, info_dict: dict, resumed: bool = True) -> None:
        ...
        Args:
            ...
            resumed (bool): Whether the corresponding ``HistoryBuffer``
                could be resumed.
        """
        assert isinstance(info_dict, dict), "`log_dict` must be a dict!, " f"but got {type(info_dict)}"
    ...
    def get_scalar(self, key: str) -> HistoryBuffer:
        ...
        if key not in self.log_scalars:
            raise KeyError(
                f"{key} is not found in Messagehub.log_buffers: " f"instance name is: {MessageHub.instance_name}"
            )
        ...
        for key, value in self._runtime_info.items():
            if self._resumed_keys.get(key, False):
                try:
                    ...
                    print_log(
                        f"{key} in message_hub cannot be copied, " f"just return its reference. ",
                        logger="current",
    ...
    def load_state_dict(self, state_dict: Union["MessageHub", dict]) -> None:
        ...
        if isinstance(state_dict, dict):
            for key in ("log_scalars", "runtime_info", "resumed_keys"):
                assert key in state_dict, "The loaded `state_dict` of `MessageHub` must contain " f"key: `{key}`"
            ...
            for key, value in state_dict["log_scalars"].items():
                if not isinstance(value, HistoryBuffer):
                    print_log(
                        f"{key} in message_hub is not HistoryBuffer, " f"just skip resuming it.",
                        logger="current",
                        level=logging.WARNING,
                    )
                    continue
            ...
            for key, value in state_dict["resumed_keys"].items():
                if key not in set(self.log_scalars.keys()) | set(self._runtime_info.keys()):
                    print_log(
                        f"resumed key: {key} is not defined in message_hub, " f"just skip resuming this key.",
                        logger="current",
                        level=logging.WARNING,
                    )
                    continue
    ...
    def _parse_input(self, name: str, value: Any) -> OrderedDict:
        ...
        if value is None:
            ...
            raise TypeError(f"{name} should be a dict or `None`, but " f"got {type(name)}")
