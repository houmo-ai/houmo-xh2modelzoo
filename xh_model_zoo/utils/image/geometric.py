...
#   This module provides functions for geometric operations
#   including resize, rotate, flip, crop, and padding.
#
...
import cv2
import numpy as np
from xhquant.utils import to_2tuple
...
def bbox_scaling(bboxes: np.ndarray, scale: float, clip_shape: Optional[Tuple[int, int]] = None) -> np.ndarray:
...
def impad_to_multiple(img: np.ndarray, divisor: int, pad_val: Union[float, List] = 0) -> np.ndarray:
...
def cutout(
    ...
    if isinstance(pad_val, (int, float)):
        ...
        ), "Expected the num of elements in tuple equals the channels" "of input image. Found {} vs {}".format(
            len(pad_val), channels
        )
    else:
...
def _get_shear_matrix(magnitude: Union[int, float], direction: str = "horizontal") -> np.ndarray:
...
def _get_translate_matrix(offset: Union[int, float], direction: str = "horizontal") -> np.ndarray:
