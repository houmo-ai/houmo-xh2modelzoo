...
# File: io.py
# Description:
#   Image I/O utilities for various formats.
#   This module provides functions for reading and writing images
#   in different formats using various backends.
#
    ...
    *,
    ...
    Args:
        ...
        backend (str | None): The image decoding backend type. Options are
            ...
            ``use_backend()`` will be used. Default: None.
    ...
    Examples:
        ...
        ...     backend='cv2')
        >>> img = imread(img_path, flag='color', channel_order='bgr',
        ...     backend='pillow')
        >>> s3_img_path = 's3://bucket/img.jpg'
        >>> # infer the file backend by the prefix s3
        >>> img = imread(s3_img_path)
        >>> # manually set the file backend petrel
        >>> img = imread(s3_img_path, backend_args={
        ...     'backend': 'petrel'})
        >>> http_img_path = 'http://path/to/img.jpg'
        >>> img = imread(http_img_path)
        >>> img = imread(http_img_path, backend_args={
        ...     'backend': 'http'})
    """
...
def imfrombytes(
    ...
    Args:
        ...
        backend (str | None): The image decoding backend type. Options are
            ...
            None, the global imread_backend specified by ``use_backend()``
    ...
    Examples:
        ...
        >>>     img_buff = f.read()
        >>> img = imfrombytes(img_buff)
        >>> img = imfrombytes(img_buff, flag='color', channel_order='rgb')
        >>> img = imfrombytes(img_buff, backend='pillow')
        >>> img = imfrombytes(img_buff, backend='cv2')
    """
...
def imwrite(
    ...
    *,
    ...
    Examples:
        ...
        >>> ret = imwrite(img, '/path/to/img.jpg')
        >>> # infer the file backend by the prefix s3
        >>> ret = imwrite(img, 's3://bucket/img.jpg')
        >>> # manually set the file backend petrel
        >>> ret = imwrite(img, 's3://bucket/img.jpg', backend_args={
        ...     'backend': 'petrel'})
    """
    if file_client_args is not None:
        warnings.warn(
