...
# File: photometric.py
# Description:
#   Photometric image transformations.
#   This module provides functions for image normalization,
#   denormalization, and other photometric operations.
#
...
from xhquant.utils import is_tuple_of
...
def adjust_color(img, alpha=1, beta=None, gamma=0, backend=None):
    ...
    Args:
        ...
        backend (str | None): The image processing backend type. Options are
            ...
            ``imread_backend`` specified by ``use_backend()`` will be
    ...
    if backend not in ["cv2", "pillow"]:
        raise ValueError(f"backend: {backend} is not supported." f"Supported backends are 'cv2', 'pillow'")
...
def adjust_brightness(img, factor=1.0, backend=None):
...
def adjust_contrast(img, factor=1.0, backend=None):
...
def adjust_hue(img: np.ndarray, hue_factor: float, backend: Optional[str] = None) -> np.ndarray:
