...
#   This module provides Config and ConfigDict classes for managing
#   configuration files with support for inheritance and lazy loading.
#
...
def add_args(parser: ArgumentParser, cfg: dict, prefix: str = "") -> ArgumentParser:
    ...
    for k, v in cfg.items():
        if isinstance(v, str):
            ...
            print_log(f"cannot parse key {prefix + k} of type {type(v)}", logger="current")
    return parser
...
class Config:
    ...
    def __init__(
        ...
        super().__setattr__("_sort_items", True)
        # self._sort_items = True
        ...
        for idx, node in enumerate(nodes):
            if (
                ...
                raise ConfigParsingError(
                    ...
                    "mark the inherited config file."  # noqa: E501
                )
    ...
    def _substitute_env_variables(filename: str, temp_config_name: str):
        ...
        for var_name, value in keys:
            ...
            if var_name in os.environ:
                ...
                print_log(
                    f"Using env variable `{var_name}` with value of " f"{value} to replace item in config.",
                    logger="current",
                )
            if not value:
                raise KeyError(
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_config_dir:
                ...
                if use_environment_variables:
                    env_variables = Config._substitute_env_variables(temp_config_file.name, temp_config_file.name)
    ...
    def _dict_to_config_dict(cfg: dict, scope: Optional[str] = None, has_scope=True):
        ...
        if isinstance(cfg, dict):
            ...
            cfg = tuple(Config._dict_to_config_dict(_cfg, scope, has_scope=has_scope) for _cfg in cfg)
        elif isinstance(cfg, list):
    ...
    def _get_base_files(filename: str) -> list:
        ...
        if file_format == ".py":
            ...
            raise ConfigParsingError("The config type should be py, json, yaml or " f"yml, but got {file_format}")
    ...
    def _get_cfg_path(cfg_path: str, filename: str) -> Tuple[str, Optional[str]]:
        ...
        if "::" in cfg_path:
            ...
            if not is_installed(package):
                raise ModuleNotFoundError(f"{package} is not installed, please install {package} " f"manually")
    ...
    def pretty_text(self) -> str:
        ...
        def _format_dict(input_dict, outest_level=False):
            ...
            if self.sort_items:
                input_dict_items = sorted(input_dict.items(), key=lambda x: str(x[0]))
            else:
                input_dict_items = input_dict.items()
            for idx, (k, v) in enumerate(input_dict_items):
        ...
        for node in ast.walk(parsed_codes):
            ...
            if isinstance(node, ast.ImportFrom):
                ...
                if node.level != 0:
                    return True
                # Skip checking
                if node.module == "xhquant" and len(node.names) == 1 and node.names[0].name == "Config":
                    continue
    ...
    def to_dict(self, keep_imported: bool = False):
        ...
        if hasattr(self, "_imported_names") and not keep_imported:
            cfg_dict = {key: value for key, value in cfg_dict.items() if key not in self._imported_names}
        return cfg_dict
