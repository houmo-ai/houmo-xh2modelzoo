...
def _get_package_and_cfg_path(cfg_path: str) -> Tuple[str, str]:
    ...
    if len(package_cfg) > 2:
        raise ValueError(
            "`::` should only be used to separate package and " "config name, but found multiple `::` in " f"{cfg_path}"
        )
...
def _is_builtin_module(module_name: str) -> bool:
    ...
    if module_name.startswith("."):
        return False
    if module_name.startswith("xhquant.utils.config"):
        return True
    ...
    if "site-package" in origin_path or "dist-package" in origin_path or not origin_path.startswith(PYTHON_ROOT_DIR):
        return False
    else:
        return True
