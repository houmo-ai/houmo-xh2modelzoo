...
# File: lazy.py
# Description:
#   Lazy loading utilities for configuration parsing.
#   This module provides LazyObject class for lazy module initialization
...
class LazyObject:
    ...
    def __init__(
        ...
        if not isinstance(module, str) and not is_seq_of(module, str):
            raise TypeError("module should be `str`, `list`, or `tuple`" f"but got {type(module)}")
        self._module: Union[str, list, tuple] = module

        if not isinstance(imported, str) and imported is not None:
            raise TypeError("imported should be `str` or None, but got " f"{type(imported)}")
